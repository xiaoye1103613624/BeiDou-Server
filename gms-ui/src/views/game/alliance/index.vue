<template>
  <div class="container">
    <Breadcrumb />
    <a-card class="general-card" :title="$t('menu.growth.alliance')">
      <a-row>
        <a-col>
          <a-space>
      ...
      >
        <template #columns>
          <a-table-column
            :title="$t('alliance.column.id')"
            data-index="id"
            :width="50"
            align="center"
          />
          <a-table-column
            :title="$t('alliance.column.name')"
            data-index="name"
            :width="140"
          />
          <a-table-column
            :title="$t('alliance.column.guildCount')"
            :width="80"
            align="center"
          >
            <template #cell="{ record }">
              <a-link @click="showGuilds(record)">{{
                record.guildCount || 0
              }}</a-link>
            </template>
          </a-table-column>
          <a-table-column
            :title="$t('alliance.column.capacity')"
            data-index="capacity"
            :width="70"
            align="center"
          />
          <a-table-column
            :title="$t('alliance.column.notice')"
            data-index="notice"
            :width="180"
            ellipsis
            tooltip
          ...
          <a-table-column
            :title="$t('alliance.column.operation')"
            :width="100"
            fixed="right"
            align="center"
          >
            <template #cell="{ record }">
              <a-popconfirm
                :content="$t('alliance.delete.confirm')"
    ...
    <!-- 下属公会弹窗 -->
    <a-modal
      v-model:visible="guildVisible"
      :title="$t('alliance.guilds.title')"
      :footer="false"
      :width="400"
    >
      <a-table
      ...
      >
        <template #columns>
          <a-table-column
            :title="$t('alliance.guilds.id')"
            data-index="guildId"
            :width="80"
            align="center"
          />
          <a-table-column
            :title="$t('alliance.guilds.name')"
