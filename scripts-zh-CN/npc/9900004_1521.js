...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "        "+爱心+" #e【 #r" + cm.getServerName() + "#k 】每日任务列表#n "+爱心+"\r\n\r\n"
			...
			if(cm.getBossLog('每日老板') == 0){
					...
					text += "		  "+琴符+"#e   (#rlv.15#k)主线任务10   "+琴符+"#l\r\n\r\n"//3
			}
			if(cm.getBossLog('每日天狗') == 0){
					...
					text += "		  "+琴符+"#e   (#rlv.15#k)主线任务11   "+琴符+"#l\r\n\r\n"//3
			}
			if(cm.getBossLog('每日大姐大') == 0){
					...
					text += "		  "+琴符+"#e   (#rlv.15#k)主线任务12   "+琴符+"#l\r\n\r\n"//3
			}
	    ...
	    cm.dispose();
			...
			if (cm.haveItem(4000040,1)){
				...
				//Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(3, cm.getClient().getChannel(), "『每日任务』" + " : " + " 恭喜玩家："+cm.getName()+" 完成每日殴打蘑菇王！奖励金币50W！")); 
