    ...
    if (mode == -1) {
		...
		if (status == 0) {
			...
			if(sel_type == 0){
				...
				selStr += "\t     目前销售收益点：#r#e"+cm.getmoneym()+"#n#b 剩余元宝：#r#e"+getmoneyb()+"#n#b\r\n\t ( 成功销售物品系统将会收取5%的服务费 )\r\n    ( 能交易的物品和装备，任务物品不予上架 )\r\n";
		...
		//} else if (sel_type == 666) {
			...
			if(sel_type == 0){
				...
				if(sr_type == 0){
					if(cm.getPotion(1) >= 100000){//判断元宝数量
					...
					if(getmoneyb() >= totel_price){
						if(cm.checkHold(select_good.getGood_id(), selection)){
							if(select_good.gainGood_num(-selection)){
								gainmoneyb(parseInt(-totel_price));
								cm.gainmoneym(select_good.getAcc_id(), parseInt(totel_price*0.95));
					...
					if(getmoneyb() >= totel_price){
						if(cm.checkHold(select_good.getGood_id(), selection)){
							if(select_good.gainEq_num(-selection)){
								...
								Packages.server.MapleInventoryManipulator.addFromDrop(cm.getClient(), newEq, false);
			...
			if(sel_type == 1){
				if(sr_type == 1){
					...
					sj_price = selection;
					Packages.server.MapleInventoryManipulator.removeFromSlot(cm.getClient(), Packages.client.inventory.MapleInventoryType.EQUIP, 1, 1, false);
