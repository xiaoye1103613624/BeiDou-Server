...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		text = "\t#e========欢迎来到	#r" + cm.getChannelServer().getServerName() + "#k========\r\n\#n功能介绍说明请参考普通星之力系统！\r\n#e本系统需要持有#v2530000#方可进行强化！\r\n#r使用X张#k幸运日卷轴可以获得以下效果：\r\n保护#r6+X#k星星等级以内#b失败时星之力不会下降#k\r\n提升#rX*10%#k的强化成功率\r\n失败时回赠#v3992025##r100个*X\r\n#rPS:仙器-魔器-神器,无法使用星之力\r\n#b";
        text += "#n- 首先请选择要强化星之力的道具 -#n\r\n#b";
        var indexof = 1;
        ...
        cm.sendSimple(text);
			...
			num = selection;
			cm.sendGetNumber("请问本次强化你想使用#v2530000#几张呢？（目前支持单次上限2张）", 1, 1, 2);
		}else if (status == 2){
			xyr = selection;
			...
			if(newsj <= (Math.floor(100/(xxdj+1))+10*xyr)){
				...
				if((shibai-xyr) <= 0){
					...
					if(shibai > 0){
					cm.sendOk("强化失败了,由于幸运日卷轴的影响，星之力并没有下降！");
					cm.dispose();
					}else {
					...
					item.setOwner("★★★★★★");
					MapleInventoryManipulator.removeFromSlot(cm.getClient(),type,num,1, false);
			...
			MapleInventoryManipulator.addFromDrop(cm.getClient(), item,false);
			cm.sendOk("星之力强化成功！");
			Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(9,cm.getClient().getChannel(),"[进阶星之力]" + " : " + " 玩家 [" + cm.getPlayer().getName() + "]成功强化星之力至"+xxnew+"！大家祝贺他(她)吧！",true).getBytes()); //喇叭  
			cm.dispose();
		}
	}
}
