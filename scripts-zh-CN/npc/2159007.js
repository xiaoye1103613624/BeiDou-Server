...
function action(mode, type, selection) {
    ...
    if (cm.getInfoQuest(23007).indexOf("vel01=2") == -1) {
	    ...
	    cm.sendNextPrev("Now, there''s nothing stopping you right? Let''s get out of here!", 2);
        } else if (status == 2) {
	    cm.sendNextPrev("Then hurry up! Let''s go!", 2);
        } else if (status == 3) {
	    ...
	    cm.sendNextPrev("This is the road that leads to Edelstein, where I live! Let''s get out of here before the Black Wings follow us.", 2);
        } else if (status == 2) {
