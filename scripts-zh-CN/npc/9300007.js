...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		/*if (!cm.getPlayer().getGender() == 1) {
			cm.sendOk("队长必须是女性,因为在冒险岛世界女性地位是很高的!");
            cm.dispose();
        ...
        cm.sendNextPrev("但是要离婚请在背包放结婚戒指，而且要付更多的钱，因为离婚不是那么容易的事情。离婚费要100万金币+1W点券。");
    } else if (status == 13) {
        cm.sendNextPrev("离婚需要100万金币+1W点券，法海师傅就会帮你处理离婚事宜。提醒一下离婚后，结婚戒指会同时消失。");
        cm.dispose();
    } else if (status == 14) {
        //to do
    } else if (status == 15) {
