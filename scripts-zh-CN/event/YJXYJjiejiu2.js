...
var entryMap = 101073110;                    // 事件启动时玩家进入的初始地图
var exitMap = 101073100;                     // 玩家未能完成事件时被传送至此地图（准备地图）
var recruitMap = exitMap;                    // 玩家必须在此地图上才能开始此事件
var clearMap = exitMap;                      // 玩家成功完成事件后被传送至此地图

var mapIds = [101073110];                    // 事件涉及的地图ID列表
var prepareMap = 101073100;                  // 准备地图（退场传送目标）

var minMapId = entryMap;                     // 事件发生在此地图ID区间内
var maxMapId = entryMap;

var eventTime = 10;                          // 事件的最大允许时间，以分钟计
var killTarget = 20;                         // 通关所需击杀怪物数量
var completeQuestId = 32124;                 // 通关时完成的任务ID ← 请确认实际任务ID
 ...
 * 在服务端日志中搜索 [妖精学院_解救2] 即可看到完整调用链路。
 */
function log(msg) {
    java.lang.System.out.println("[妖精学院_解救2] " + msg);
}
...
function logErr(msg, err) {
    java.lang.System.out.println("[妖精学院_解救2] ERROR: " + msg);
    if (err) {
        java.lang.System.out.println("[妖精学院_解救2] " + err.toString());
        if (err.stack) {
            java.lang.System.out.println("[妖精学院_解救2] " + err.stack);
        }
    }
}
...
function end(eim) {
    ...
    // 用 try-catch 包裹退场逻辑，防止 changedMap 中已 unregister 的玩家
...
function monsterKilled(mob, eim) {
    ...
    if (count >= killTarget) {
        ...
        for (var i = 0; i < players.size(); i++) {
            ...
            try {
                p.dropMessage(5, "击杀目标达成！请等待自动退场。");
            } catch (msgErr) {
