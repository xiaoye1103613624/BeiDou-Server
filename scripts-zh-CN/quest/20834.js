...
function start(mode, type, selection) {
	...
	if (status == 0) {
	    ...
	    qm.sendNextPrev("My name's #h #. I love Ereve. It's so pretty here.");
	} else if (status == 2) {
      qm.sendNextPrev("Oh, yes. Ereve is lovely, and so peaceful. Has your training been difficult?");
	} else if (status == 3) {
	    qm.sendNextPrev("Nothing I can't handle. Maple World better get ready, because I'm about to save it so hard.");
	} else if (status == 4) {
	    ...
	    qm.sendNextPrev("Yeah, um, I'm sorry. Who are you again? You don't look like much of a knight...");
	} else if (status == 6) {
