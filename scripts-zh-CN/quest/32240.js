...
function start(mode, type, selection) {
    ...
    if (status == 0) {
	    ...
	    qm.sendNextPrev("Why introduce ourselves now? Don't ask stupid questions.",4);//not sure
	} else if (status == 2) {
	    qm.sendNextPrev("I came to give you a gift. It's an #bExplorer Book#k, kinda like a diary. In this, you can record every exciting adventure you'll ever have! And then I can read about it later!",4);//not sure
	} else if (status == 3) {
	    ...
	    qm.sendNextPrev("Found it!  Here. Take a good look at it after I take off.",4);//not sure
	} else if (status == 6) {
...
function end(mode, type, selection) {
	...
	if (status == 0) {
	    ...
	    qm.sendNextPrev("I guess it's been a while since I first started my journey. Looking back, I...",16);//not sure
	} else if (status == 3) {
	    qm.sendNextPrev("#b#L0#Trained hard to level up.#l\r\n#L1#Worked hard to make money.#l\r\n#L2#Partied hard to make new friends.#l\r\n#L3#Didn't do much at all.#l",4);//not sure
	} else if (status == 4) {
	    ...
	    qm.sendNextPrev("Oh well, the past is past. I should focus on future. New adventures! New horizons! ...Huh?",16);//not sure
	} else if (status == 6) {
