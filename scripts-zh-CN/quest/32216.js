...
function start(mode, type, selection) {
    ...
    if (status == 0) {
	    ...
	    qm.sendNextPrev("#bHuh? Job?#k",17);
	} else if (status == 2) {
	    qm.sendNextPrev("There are five different Explorer Jobs. You can advance to them on Victoria Island. Hm, I think they were... Warrior, Magician, Bowman, Thief, and Pirate.",1);
	} else if (status == 3) {
		qm.sendNextPrev("#bWhat are they like?#k",17);
	} else if (status == 4) {
	    qm.sendNextPrev("Let's see. Warriors have great strength and defense, so they excel at close-range combat. Magicians use magic, so they f-favor intelligence over power, and they're good at long-range combat against multiple enemies.",1);
	} else if (status == 5) {
	    qm.sendNextPrev("B-bowman are also good at long-range combat. They shoot arrows from afar and can keep enemies at a distance. And, let's see... Thieves are close-range, like warriors, but they focus on speed instead of strength.",1);
	} else if (status == 6) {
	    qm.sendNextPrev("Finally, Pirates... are Pirates. Some use their fists in close-range combat, others shoot guns or cannons from afar. Their attacks are pretty fancy, either way.",1);
	} else if (status == 7) {
