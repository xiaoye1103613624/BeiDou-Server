 ...
 * Addon −54…−61 blind (sidecar arena on vanilla 52-slot aEquipped); pocket −33 / Si −10
 * never blind (UI = classic Equip); aux −62 sidecar (UI = classic red10); cash rings never −152/−153.
...
public final class ExtendedEquipRegistry {
    ...
    public static final List<Seat> SEATS = Collections.unmodifiableList(Arrays.asList(
            new Seat(20, 115, "Sh", UiSeat.MAIN_EQUIP, StorageTier.NATIVE_APPLY_MAX_55,
                    true, DrawPolicy.REMAP_XY, WireWhen.ALWAYS, new short[]{-20},
                    "shoulder (STAT blind; keep client −20 skip)"),
            ...
            new Seat(62, 134, "Aw", UiSeat.MAIN_EQUIP, StorageTier.SIDECAR_ZREF,
                    true, DrawPolicy.REMAP_XY, WireWhen.ALWAYS, new short[]{-62, -162}, "134/135 aux classic red10"),
    ...
    /**
     * <b>NATIVE CD64 LIVE</b> — set true ONLY after CD64 EXE (Route A,
     * {@code push 0x700 @778F02} + GetItem bound −63 + apply-max 62) is
     * enter-green on the live client.
     * <p>
     * Once native, the client folds −52..−62 (and their cash mirrors) into its
     * own 四维/MAXHP/MAXMP via the EXE-grown walkers; server blind-injection for
     * those seats must flip OFF or stats are double-counted.
     * <p>
     * Default {@code false} (live is vanilla EXE + sidecar — keep blind).
     * Flip together with the client sidecar detach (avoid S2/S7 in
     * {@code NATIVE_EXTEND_PLAN_CLIENT1_20260807.md}).
     */
    public static final boolean NATIVE_CD64_LIVE = false;

    /**
     * ADDON_SERVER_STATS — Occ-OFF seats injected via STAT_CHANGED.
     * Never list Si −10 or pocket −33 (client-native fold).
     * <p>
     * On {@link #NATIVE_CD64_LIVE} the native seats are client-native again
     * (CD64 EXE grown aEquipped) → injection disabled for −52..−62/−154..−162.
     */
    public static boolean isClientBlindEquipSlot(short pos) {
        if (NATIVE_CD64_LIVE
                && ((pos >= -62 && pos <= -52) || (pos >= -162 && pos <= -154))) {
            return false;
        }
        return switch (pos) {
            // Shoulder −20: v083 still jz-skips −20 in combat/mainstat fold even with
