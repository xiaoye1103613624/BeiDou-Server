...
/**
 * 【业务服务】EquipAdvanceService：装备进阶服务类，负责装备进阶系统的配置管理。
 *
 * <p>提供装备进阶路线的增删改查操作，包括路线配置、阶段属性、消耗材料等，
 * 并在配置变更时同步更新 {@link EquipAdvanceManager} 的内存缓存。</p>
 */
@Slf4j
@Service
@AllArgsConstructor
public class EquipAdvanceService {
     ...
     * 从数据库加载所有装备进阶配置并初始化到 {@link EquipAdvanceManager}。
     */
    @PostConstruct
    public void init() {
        refreshCache();
    ...
    /**
     * 获取所有装备进阶路线列表。
     * @return 进阶路线DTO列表，包含路线、阶段和消耗材料信息
     */
    ...
    public EquipAdvanceSaveDTO getRouteById(Long id) {
        ...
        List<EquipAdvanceStageDO> allStages = stageMapper.selectAll();
        List<EquipAdvanceCostDO> allCosts = costMapper.selectAll();
        return toDTO(route, allStages, allCosts);
    }
    ...
    /**
     * 刷新缓存，将数据库最新配置加载到 {@link EquipAdvanceManager} 内存中。
     */
