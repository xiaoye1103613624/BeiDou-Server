...
public final class ScrollHandler extends AbstractPacketHandler {
    ...
    private static void handlePotentialOrHyper(Client c, Character chr, Equip equip, Item scroll,
        ...
        if (result == org.gms.potential.PotentialHyperService.Result.SUCCESS) {
            ...
            if (usesMiracleCubeWindow(scrollId)) {
                org.gms.potential.PotentialHyperService.sendMiracleCubeResultWindow(chr, equip, scrollId, true);
            }
            if (equipSlot >= 0) {
    ...
    private static boolean usesMiracleCubeWindow(int scrollId) {
        return org.gms.potential.PotentialHyperConfig.isMainCube(scrollId)
                || org.gms.potential.PotentialHyperConfig.isPremiumCube(scrollId)
                || org.gms.potential.PotentialHyperConfig.isSuperCube(scrollId)
                || org.gms.potential.PotentialHyperConfig.isUltimateCube(scrollId)
                || org.gms.potential.PotentialHyperConfig.isWeirdCube(scrollId)
                || org.gms.potential.PotentialHyperConfig.isBonusCube(scrollId);
    }
