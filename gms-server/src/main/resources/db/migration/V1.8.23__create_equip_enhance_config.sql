...
-- 装备强化等级配置（每个等级定义成功率、消耗、属性加成）
CREATE TABLE IF NOT EXISTS `xy_equip_enhance_level`
(
    `id`              BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '自增主键',
    `config_id`       BIGINT NOT NULL COMMENT '关联 xy_equip_enhance_config.id',
    `enhance_level`   INT    NOT NULL COMMENT '强化等级（1~N）',
    `success_rate`    INT    NOT NULL DEFAULT 100 COMMENT '成功率（0~100）',
    `destroy_on_fail` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '失败是否销毁装备（0=保留 1=销毁）',
...
-- 装备强化道具消耗（每级可配置多种消耗道具）
CREATE TABLE IF NOT EXISTS `xy_equip_enhance_cost`
(
    `id`          BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '自增主键',
    `level_id`    BIGINT NOT NULL COMMENT '关联 xy_equip_enhance_level.id',
