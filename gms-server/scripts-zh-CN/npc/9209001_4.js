...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("\t\t\t #v2640010#特殊卷轴#v2640011#\r\n\r\n在我这里可以使用特殊卷轴对装备进行强化!\r\n#r\r\n#b将会消耗强化次数#r1次,#b请将需要强化的装备放在#r第一格!\r\n注意：强化后产生的效由卷轴决定！#k\r\n");
		} else if (status == 1){
			var cc = cm.getInventory(1).getItem(1).getItemId();
        text = "#r  -请选择要使用的卷轴- #n   当前第一格物品为：#v"+cc+"#\r\n\r\n#k#n";
		text += "\r\n#L1##v2640010##z2640010#";
		text += "\r\n#L2##v2640011##z2640011#";
		text += "\r\n\r\n#L3##v2615001##z2615001#";
        ...
        cm.sendSimple(text);
		    ...
		    if(cm.getInventory(1).getItem(1).getExpiration() != -1) {
				...
				if(sj !=0){
					...
					MapleInventoryManipulator.removeFromSlot(cm.getClient(),type,1,1, false);
					MapleInventoryManipulator.addFromDrop(cm.getClient(), item,false);
					Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(9,cm.getClient().getChannel(),"『强化公告』" + " : " + " 玩家 [" + cm.getPlayer().getName() + "]在自由市场饮水机使用卷轴强化成功！",true).getBytes()); //喇叭  
					...
					cm.gainItem(2640010,-1);
					cm.gainItem(2340000,-1);
					Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(9,cm.getClient().getChannel(),"『强化公告』" + " : " + " 玩家 [" + cm.getPlayer().getName() + "]在自由市场饮水机使用卷轴强化失败！",true).getBytes()); //喇叭  
					cm.dispose();
				}
			}
