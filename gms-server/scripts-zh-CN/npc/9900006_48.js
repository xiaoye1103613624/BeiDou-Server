...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            text += "\t#L48##b#v2280003#【冒险岛勇士】   #L49##b#v2280003#【点卷兑换】#l\r\n\r\n"//3
			
			
			text += "\t#L50##b#v2280003#【洗血兑换】   #L51##b#v2280003#【矿石兑换】#l\r\n\r\n"//3
            ...
            cm.sendSimple(text);
				...
				cm.dispose();
				cm.openNpc(9310072, 50);
        } 
		else if (selection == 49) {//游戏中心
				cm.dispose();
				cm.openNpc(9310072, 49);
        } 
			...
			if(cm.getPlayer().getMeso() >= 100 ){ //物品条件
				...
				Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(3, cm.getClient().getChannel(), "『兑换中心』" + " : " + "[" + cm.getChar().getName() + "]兑换了1本冒险岛勇士技能书！")); 
		        cm.dispose();
			}else{
				cm.sendOk("\t点券不足。");
				cm.dispose();
