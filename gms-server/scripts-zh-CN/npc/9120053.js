...
function start() {
    if (cm.getMapId() == 802000800) {
        ...
        if (prop == null || prop.equals("0")) {
            var exped = cm.getExpedition(ExpeditionType.CORE_BLAZE);
            if (exped == null) {
                status = 0;
                cm.sendYesNo("你想成为远征队长吗？");
            } else if (cm.isLeaderExpedition(ExpeditionType.CORE_BLAZE)) {
                status = 10;
                cm.sendSimple("你想做什么?远征队长。 \r\n#b#L0#查看远征队#l \r\n#b#L1#制裁远征队员#l \r\n#b#L2#查看制裁名单#l \r\n#r#L3#开始远征任务#l");
            } else {
                var memberType = exped.getMembers().containsKey(cm.getPlayer().getId());
...
function action(mode, type, selection) {
    ...
    case 0:
        if (mode == 1) {
            var result = cm.createExpedition(ExpeditionType.CORE_BLAZE);
            if (result == 0) {
    ...
    case 5:
        ...
        var exped5 = cm.getExpedition(ExpeditionType.CORE_BLAZE);
        if (exped5 == null) {
    ...
    case 10:
        ...
        var exped10 = cm.getExpedition(ExpeditionType.CORE_BLAZE);
        if (exped10 == null) {
            cm.sendOk("远征队已经注销，请重新发起。");
            cm.dispose();
            break;
    ...
    case 11:
        if (mode == 1 && selection >= 0) {
            var exped11 = cm.getExpedition(ExpeditionType.CORE_BLAZE);
            if (exped11 != null) {
    ...
    case 12:
        if (mode == 1 && selection >= 0) {
            var exped12 = cm.getExpedition(ExpeditionType.CORE_BLAZE);
            if (exped12 != null) {
