...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		if(!cm.haveItem(材料1,材料1数量)){
			cm.sendOk("材料强化系统使用需要以下材料:\r\n#v"+材料1+"#x"+材料1数量+"");
            cm.dispose();
            return;
		} else if(!cm.haveItem(材料2,材料2数量)){
			cm.sendOk("材料强化系统使用需要以下材料:\r\n#v"+材料2+"#x"+材料2数量+"");
            cm.dispose();
            return;
		} else if(!cm.haveItem(材料3,材料3数量)){
			cm.sendOk("材料强化系统使用需要以下材料:\r\n#v"+材料3+"#x"+材料3数量+"");
            cm.dispose();
            return;
		} else if(cm.getMeso() < 冒险币){
			cm.sendOk("材料强化系统使用需要以下材料:\r\n冒险币 = "+冒险币+"");
            cm.dispose();
            return;
		} else if(cm.getBossLog("每日材料强化") >= 8){
			cm.sendOk("每日只能强化8次哟");
            cm.dispose();
            return;
		} else {
		...
		cm.getInventory(1).getItem(1).setMatk(魔法+s6);
		cm.刷新状态();
