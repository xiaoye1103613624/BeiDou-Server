...
function action(mode, type, selection) {
    ...
    status++;
    if (status == 0) {
        cm.sendSimple("嗨， #r#h ##k! 我的名字是 #r#p9000039##k. 如果你有 #b充值点数#k. 可以找我换东西哦。 好了你需要什么？？\r\n#b#L0#什么是充值点数？#l\r\n#b#L1#换一些充值商品#l \r\n#b#L2#换一些椅子#l \r\n#b#L3#换一些名声#l\r\n#L4#换一些点数#l\r\n#L6#我目前有多少充值点数?\r\n#L5#红色福袋换1千充值点#l \r\n#L7#1千充值点换红色福袋#l#k");
    } else if (status == 1) {
        sel = selection;
        if (selection == 0) {
            cm.sendNext("充值点数可以买一些物品，至于怎么样有充值点数...这要问\r\nGM呢=)");
            status = -1;
        } else if (selection == 1) {
            var selStr = "你想要什么？？#b\r\n\r\n";
            for (var i = 0; i < items.length; i++) {
                selStr += "#L" + i + "##v" + items[i] + "##t" + items[i] + "#" + (itemsu[i] > 0 ? "(附赠 " + itemsu[i] + " 卷)" : "") + " x " + itemsq[i] + " 为 #e" + itemsp[i] + "#n 充值点#n" + (itemse[i] > 0 ? (" ...期限 #r#e" + itemse[i] + "#n#b天") : "") + "#l\r\n";
            }
            cm.sendSimple(selStr + "#k");
        } else if (selection == 2) {
            var selStr = "你想要什么？？#b\r\n\r\n";
            for (var i = 0; i < chairs.length; i++) {
                selStr += "#L" + i + "##v" + chairs[i] + "##t" + chairs[i] + "# 为 #e" + chairsp[i] + "#n 充值点#n#l\r\n";
            }
            cm.sendSimple(selStr + "#k");
        } else if (selection == 3) {
            cm.sendYesNo("你需要名声?? 我可以跟你交换 #b#e" + fame + " 名声为 " + famep + " 充值点.#n#k. 请问你接受吗??");
        } else if (selection == 4) {
            cm.sendYesNo("你需要GASH对吧?? 我可以跟你交换 #r#e1000 GASH 为 " + acashp + " 充值点.#n#k. 请问你接受吗??");
        } else if (selection == 5) {
            if (!cm.getPlayer().haveItem(3993003)) {
                cm.sendOk("你需要有一个#b#t3993003##k。");
                cm.dispose();
            } else {
                cm.sendGetNumber("你想要用多少#r#t3993003##k换充值点数？？ \r\n比值: (1 #t3993003# = 1000 充值点) \r\n(目前你有: " + cm.getPlayer().itemQuantity(3993003) + "个 #r#t3993003##k) \r\n(目前你有: " + cm.getPlayer().getPoints() + "充值点)", cm.getPlayer().itemQuantity(3993003), 1, cm.getPlayer().itemQuantity(3993003));
            }
        } else if (selection == 7) {
            if (cm.getPlayer().getPoints() < 1000) {
                cm.sendOk("你需要有#b一千充值点#k。");
                cm.dispose();
            } else {
                cm.sendGetNumber("你想要用多少#r充值点数#k换#t3993003#？？ \r\n比值: (1 #t3993003# = 1000 充值点) \r\n(目前你有: " + cm.getPlayer().getPoints() + "充值点) (目前你有:  " + cm.getPlayer().itemQuantity(3993003) + "个 #r#t3993003##k)", cm.getPlayer().getPoints() / 1000, 1, cm.getPlayer().getPoints() / 1000);
            }
        } else if (selection == 6) {
            cm.sendOk("目前你有 #e" + cm.getPlayer().getPoints() + "#n 充值点数。");
            cm.dispose();
        }
    } else if (status == 2) {
        if (sel == 1) {
            ...
            if (cm.getPlayer().getPoints() < ip) {
                cm.sendOk("很抱歉，你没有足够的充值点数 你目前有 " + cm.getPlayer().getPoints() + " 我需要 " + ip + ".");
            ...
            if (cm.getPlayer().getPoints() < cp) {
                cm.sendOk("很抱歉，你没有足够的充值点数 你目前有 " + cm.getPlayer().getPoints() + " 我需要 " + cp + ".");
            ...
            if (cm.getPlayer().getPoints() < famep) {
                cm.sendOk("很抱歉，你没有足够的充值点数 你目前有 " + cm.getPlayer().getPoints() + " 我需要 " + famep + ".");
            ...
            if (cm.getPlayer().getPoints() < acashp) {
                cm.sendOk("很抱歉，你没有足够的充值点数 你目前有 " + cm.getPlayer().getPoints() + " 我需要 " + acashp + ".");
            ...
            if (selection >= 1 && selection <= cm.getPlayer().itemQuantity(3993003)) {
                if (cm.getPlayer().getPoints() > (2147483647 - (selection * 1000))) {
                    ...
                    cm.sendOk("你失去了 " + selection + " #r个#t3993003##k 和 获得了 " + (selection * 1000) + " 充值点. \r\n目前充值点: " + cm.getPlayer().getPoints());
                }
            }
        } else if (sel == 7) {
            if (selection >= 1) {
                if (selection > (cm.getPlayer().getPoints() / 1000)) {
                    cm.sendOk("你最多只能得到 " + (cm.getPlayer().getPoints() / 1000) + ". 1 道具 = 1000 充值点.");
                } else if (!cm.canHold(3993003, selection)) {
                    cm.sendOk("请空出一些装饰栏。");
                } else {
                    cm.gainItem(3993003, selection);
                    cm.getPlayer().setPoints(cm.getPlayer().getPoints() - (selection * 1000));
                    cm.sendOk("你获得了 " + selection + " #r个#t3993003##k 和 失去了 " + (selection * 1000) + " 充值点. \r\n目前充值点: " + cm.getPlayer().getPoints());
                }
            }
        }
        cm.dispose();
