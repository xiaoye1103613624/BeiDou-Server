	...
	else 
	{
        ...
        if (status == 0) 
		{
			...
			if(cm.getPlayer().getMapId()==180000001)
			{
				...
				add += "\r\n\r\n"
				add += "\t      #L1#"+红色箭头+蓝色箭头+"  #e#k挑战 黑魔法师（战力）#n#l\r\n";
				add += "\t      #L2#"+红色箭头+蓝色箭头+"  #e#b挑战 黑暗剑灵（破攻）#n#l\r\n";
				add += "\t      #L3#"+红色箭头+蓝色箭头+"  #e#r挑战 光明剑灵（称号）#n#l\r\n\r\n";
...
//------------------------------------------------------------------------
		...
		else if (status == 1) 
		{
			...
			{
					cm.dispose();
					cm.openNpc(2144000, "黑暗领域-黑魔法师");
			}
			if (selection == 2) 
			{
					cm.dispose();
					cm.openNpc(2144000, "黑暗领域-黑暗剑灵");
			}
			if (selection == 3) 
			{
					cm.dispose();
					cm.openNpc(2144000, "黑暗领域-光明剑灵");//这个代码三小的挑战NPC外观，9000434
			}
			if (selection == 4) 
			{
					cm.dispose();
					cm.openNpc(2144000, "黑暗领域积分商店");
			}
		}
	}
}
