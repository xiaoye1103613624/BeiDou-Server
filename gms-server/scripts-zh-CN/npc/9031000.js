...
var 列表 = [
	...
	{ 地图: 511000120, 战力要求: 800, 标题: ""+铸杌+"#k铸杌        进入需要:#r800万#k战力以上可挑战" },
	{ 地图: 511000140, 战力要求: 1000,标题: ""+混沌+"#k混沌        进入需要:#r1000万#k战力以上可挑战" },
	{ 地图: 511000160, 战力要求: 1200,标题: ""+卡琳+"#k卡琳        进入需要:#r1200万#k战力以上可挑战" },
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			text += "\t\t\t\t #d当前战力:#r " + (cm.getPlayer().GetCombat()/10000).toFixed(0) + " #k万\r\n";
			cm.sendSimple(text);
		} else if (status == 1) {
			if (selection == 999) {
				cm.dispose();
