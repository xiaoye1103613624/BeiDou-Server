...
var 领取补偿 = "补偿第3次";      //以后只需要改这里的数字-第1次
...
var 道具3 = 2049100;             //混沌卷轴60%  2张
var 数量3 = 5;

var 道具4 = 2049122;             //正向混沌
var 数量4 = 2;

var 道具5 = 2450000;             //迎春花语 4个
var 数量5 = 5;
...
function action(mode, type, selection) {
    ...
    // 检查是否在20:00至22:00之间
	if (hour < 0 || hour >= 24) {
  //  if (!((hour == 17 && minute >= 20) || (hour == 18) || (hour == 19) || (hour == 20) || (hour == 21) || (hour == 22 && minute <= 20))) {
    //    cm.sendOk("【#r老G福利#k】领取时间为#r 00:00 至 01:00#k。\r\n\r\n当前不是领取时间！！！\r\n\r\n服务器当前时间：#r" + hour +":" + minute + ":" + second + "#n#k");
		cm.sendOk("当前没有发放福利！！！");
        cm.dispose();
        return;
		}
	if (mode == -1) {
		...
		//	   text += "#k 元宝： #r["+元宝+"] #k个#l\r\n\r\n\r\n\r\n";  //元宝-----------------------------------------开区清空
			...
			//cm.gainMeso(金币);
	//		cm.setmoneyb(+元宝);
	//		cm.getPlayer().setlpjf(cm.getPlayer().getlpjf()+1000); //累计积分-----------------------------------------开区清空
			cm.gainItem(道具1,数量1);
			cm.gainItem(道具2,数量2);
			cm.gainItem(道具3,数量3);
			cm.gainItem(道具4,数量4);
		    ...
		    cm.喇叭(1,"[福利中心] 玩家 ["+cm.getName()+"] 在【市场相框-活动中心】领取了老G发放的【福利】");
			cm.喇叭(1,"[福利中心] 玩家 ["+cm.getName()+"] 在【市场相框-活动中心】领取了老G发放的【福利】");
			cm.喇叭(1,"[福利中心] 玩家 ["+cm.getName()+"] 在【市场相框-活动中心】领取了老G发放的【福利】");
			cm.dispose();
		    }
