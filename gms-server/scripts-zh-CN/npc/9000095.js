	...
	else 
	{
        ...
        if (status == 0) 
		{
			...
			if(cm.getPlayer().getMapId()==180000001)
			{
				...
				add += "\r\n"
				add += "\t          #L1#"+红色箭头+蓝色箭头+"  #k挑战 斯  乌（战力）#l \r\n";
				//add += "\t          #L2#"+红色箭头+蓝色箭头+"  #b挑战 欧碧啦（破攻）#l\r\n";
				//add += "\t          #L3#"+红色箭头+蓝色箭头+"  #r挑战 欧啦啦（称号）#l\r\n\r\n";
...
//------------------------------------------------------------------------
		...
		else if (status == 1) 
		{
			...
			{
					cm.dispose();
					cm.openNpc(9000095, "魔法庭院-斯乌");
			}
			if (selection == 2) 
			{
					cm.dispose();
					cm.openNpc(9000434, "魔法庭院-欧碧啦");
			}
			if (selection == 3) 
			{
					cm.dispose();
					cm.openNpc(9000434, "魔法庭院-欧啦啦");
			}
			if (selection == 4) 
			{
					cm.dispose();
					cm.openNpc(9000095, "魔法庭院积分商店");
			}
		}
	}
}
