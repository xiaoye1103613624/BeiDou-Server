...
function action(mode, type, selection) {
	...
	cm.sendNextPrev("Shoot, we were spotted!", 2);
    } else if (status == 2) {
	...
	cm.sendNextPrev("That''s right. I''m a resident of Edelstein, not a test subject. You can''t boss ME around.", 2);
    } else if (status == 4) {
	...
	cm.sendNextPrev("Hmph. Big words, but let''s see if you can catch me first.", 2);
    } else if (status == 6) {
	...
	cm.sendNextPrev("Hold it right there!", 4, 2159010);
    } else if (status == 9) {
	cm.warp(931000021,1);
    	cm.dispose();
    }
