...
function action(mode, type, selection) {
		...
		if (status == 0) {
			var text = "#b我是饱读诗书的孔明，答题时间为每时#r49-59分#n#k\r\n#b每日答题上限100题请认真回答每一题~#b\r\n";
			text += "#b<注意>:每天晚上零点清空答题数据#b，请及时兑换\r\n";
			text += "当前时间：#r" + hour +":" + minute + ":" + second + "#b （每题限时20秒，超时作废）\r\n#n#k\r\n\r\n";
			text += "今日答题总数: #r"+ cm.getPlayer().getBossLog("答题总数") +" #k道#n#k\r\n";
			text += "今日正确题目: #r"+ cm.getPlayer().getBossLog("答题正确") +" #k道#n#k\r\n";
			
			text += "#L1##r "+红色箭头+"点此查看答题详细规则（新手必看）#l\r\n";
			text += "#L2##r "+红色箭头+"开始答题（"+timeout+"秒限时，速度要快哦）#l\r\n";
			text += "#L3##r "+红色箭头+"答对题目数兑换/纪念币/跑环币/卷轴碎片/点卷#l\r\n";
			
			cm.sendSimple(text);
		}else if (status == 1) {
			if(selection==1){
					cm.sendOk("每次答题都有#b"+timeout+"秒钟时间限制#k，超过时间则失败，指定时间内回答错误也失败！点击开始答题后开始记时，在给出的题目中，#r一般有4个选项#k，请输入你认为正确的选项，点击确认即可完成答题.\r\n#r#e注意：每小时的49分，#b如（1点49分至1点59分）#r,都可以答题，答题时间为10分钟。每次回答正确后，将会#d\r\n随机给你奖励，如游戏币、点卷、抵用券、黄金枫叶或是金杯！");
					cm.dispose();
					return;
			}
			if(selection==2){
			...
			if(selection==3){
				...
				selText += "#L1# 换取 国庆币#v4000463# 1个     (需答对 38道)#l\r\n";
				selText += "#L2# 换取 跑环币#v3600001# 3个     (需答对 58道)#l\r\n";
				selText += "#L3# 换取 卷轴#v4032391# 20个      (需答对 68道)#l\r\n";
				selText += "#L4# 换取 卷轴#v4032392# 20个      (需答对 78道)#l\r\n\r\n";
				selText += "#L5# 换取 "+点卷图标+"点卷:66666      (需答对 88道)#l\r\n\r\n\r\n";
				selText += "      #e#r注意：每个奖励每天只能兑换一次。！\r\n";
				cm.sendSimple(selText);
			}
		}else if (status == 2) {
			if(typed == 1){
				if(cm.getPlayer().getBossLog("答题限制") > 100){
				cm.sendOk("很遗憾，你今天的100道题已经回答完毕，请用心回答每一道题不然可会浪费许多奖励喔！");
				cm.dispose();
				return;
				}else{
				cm.getPlayer().setBossLog("答题总数");
				cm.getPlayer().setBossLog("答题限制");
				...
				if(selection == tknow[5] || fee == tknow[5]){
					...
					var randdj = Math.floor(Math.random() * 100)+50;//抵用卷
					var randdy = Math.floor(Math.random() * 10) + 5;//点卷
					var randjf = Math.floor(Math.random() * 1) + 1;//黄金枫叶
					var randred = Math.floor(Math.random() * 1) + 1;//征服币
					cm.getPlayer().setBossLog("答题正确");					
					if(randx < 3){//冒险币
						cm.gainMeso(randy); 						
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 "+randy+" 游戏币.");
						cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randy+"游戏币~");
					}
					if(randx >= 3 && randx < 4){//抵用券
						cm.gainDY(randdj);
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 " +randdj+"抵用卷.");
						cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randdj+"抵用卷~");
					}
					if(randx >= 4 && randx < 9){//点劵
						cm.gainNX(randdy);
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 "+randdy+" 点卷");
						cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randdy+"点卷~");
					}
					if(randx >= 9 && randx < 12){//黄金枫叶
						//cm.gainPlayerPoints(randjf);//积分
						cm.gainItem(4000313, randjf);
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 " +randjf+" 黄金枫叶.");
						cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randjf+"黄金枫叶~");
					}
					if(randx >= 12 && randx<= 15){//金杯
						cm.gainItem(4000038, randred);
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 " +randred+" 金杯.");
					        cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randred+"金杯~");
					}					
				}else{
					cm.sendOk("很遗憾，回答错误！\r\n");
			...
			if(typed==2){
				if(selection==1){//// 38道 1国庆币4000463   
					if (cm.getPlayer().getBossLog("答题38兑换") < 1){
						if (cm.getPlayer().getBossLog("答题正确") < 38){
							cm.sendOk("抱歉，您今天还没有答对38道题目，请继续努力！\r\n");
							cm.dispose();
							return;
						}
						cm.gainItem(4000463, 1);
						cm.getPlayer().setBossLog("答题38兑换");
						cm.sendOk("恭喜，您使用正确答题数目领取成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目38题领取了 国庆币~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
					}
				} else if (selection == 2){//58道 1跑环币 3600001 
							if (cm.getPlayer().getBossLog("答题58兑换") < 1){
						if (cm.getPlayer().getBossLog("答题正确") < 58){
							cm.sendOk("抱歉，您今天还没有答对58道题目，请继续努力！\r\n");
							cm.dispose();
							return;
						}
						cm.gainItem(3600001, 3);//1跑环币
						cm.getPlayer().setBossLog("答题58兑换");
						cm.sendOk("恭喜，您使用正确答题数目领取成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目58题领取了 暗影币~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
							...
							if (cm.getPlayer().getBossLog("答题68兑换") < 1){
						if (cm.getPlayer().getBossLog("答题正确") < 68){
							cm.sendOk("抱歉，您今天还没有答对68道题目，请继续努力！\r\n");
							cm.dispose();
							return;
						}
						cm.gainItem(4032391, 20);//破碎1
						cm.getPlayer().setBossLog("答题68兑换");
						cm.sendOk("恭喜，您使用正确答题数目领取成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目68题领取了 破碎的卷轴碎片1~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
							...
							if (cm.getPlayer().getBossLog("答题78兑换") < 1){
						if (cm.getPlayer().getBossLog("答题正确") < 78){
							cm.sendOk("抱歉，您今天还没有答对78道题目，请继续努力！\r\n");
							cm.dispose();
							return;
						}
						cm.gainItem(4032392, 20);//破碎2
						
						cm.getPlayer().setBossLog("答题78兑换");
						cm.sendOk("恭喜，您使用正确答题数目领取成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目78题领取了 破碎的卷轴碎片2~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
					}
					} else if (selection == 5){//  88道勋章1142609
							if (cm.getPlayer().getBossLog("答题88兑换") < 1){
						if (cm.getPlayer().getBossLog("答题正确") < 88){
						...
						cm.getPlayer().setBossLog("答题88兑换");
						cm.getPlayer().modifyCSPoints(1,66666, true);//点券
						cm.sendOk("恭喜，您正确答题数目领取成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目88题领取了点卷66666~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
					}
				} else if (selection == 44){//换取150级武器箱子
					if (cm.getPlayer().getBossLog("答题正确") < 800){
					...
					cm.getPlayer().setBossLog("答题正确", 0, -800);
