...
function action(mode, type, selection) {
	...
	if (sel == 6) {
	    cm.sendYesNo("需要购买鱼饵吗？（180点卷/30个）");	
	    if (cm.canHold(2300000,30) && cm.getPlayer().getNX() >= 180) {
		if (!cm.haveItem(2300000)) {
		    cm.gainNX(-180);
		    cm.gainItem(2300000, 30);
		    cm.sendNext("快乐钓鱼~用完背包所有的鱼饵才能继续购买鱼饵哟");
		} else {
		    cm.sendNext("拿好您的诱饵，钓鱼快乐。");
		}
	    } else {
		cm.sendOk("请检查是否有所需的#r180#k点卷或足够的背包空间。");
	    }
	    cm.safeDispose();
	}
    }
