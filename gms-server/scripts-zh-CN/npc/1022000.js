...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getJob() == 0) {
            ...
            if (cm.getPlayer().getLevel() >= 30 && cm.getJob() == 100) { // 劍士
                if (cm.haveItem(4031012, 1)) {
                    if (cm.haveItem(4031012, 1)) {
                        status = 20;
                        cm.sendNext("我看到你完成了测试. 想要继续请点下一页!");
                    } else {
                        if (!cm.haveItem(4031008)) {
                            cm.gainItem(4031008, 1);
                        }
                        cm.sendOk("请去找 #r剑客转职教官#k.")
                        cm.dispose();
                    }
                } else {
                    status = 10;
                ...
                if (cm.canHoldByType(4, 2)) {
                    if (cm.haveItem(4031059, 1)) {
                        ...
                        cm.sendOk("嗨, #b#h0##k! 我需要一个 #b黑符#k. 快去找异次元空间拿给我");//4031059
                    }
                } else {
                    cm.sendOk("你的背包没有多余的3个空格。");
                    cm.dispose();
                    return;
                }
                cm.dispose();
        ...
        cm.sendNextPrev("你可以选择你要转职成为一位 #r剑客#k, #r准骑士#k 或 #r枪骑士#k.")
    } else if (status == 12) {
        cm.askAcceptDecline("但是我必须先测试你,你准备好了吗 ?");
    } else if (status == 13) {
        cm.gainItem(4031008, 1);
        cm.warp(102020300);
        cm.sendOk("请去找 #b剑客转职教官#k . 他会帮助你的.");
        cm.dispose();
    } else if (status == 21) {
