	...
	if (mode == -1) {
		...
		if (status == 0) {
		  ...
		  if (cm.getBossLog("qianghua1") == 20){
				...
				cm.gainItem(3992025,-3);//cm.gainNX(-5000);//扣除点卷
				if(sj >=10){
					...
					//Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(9,cm.getClient().getChannel(),"『强化公告』" + " : " + " 玩家 [" + cm.getPlayer().getName() + "]在自由市场-饮水机-强化成功！大家祝贺他(她)吧！",true).getBytes()); //
					//cm.laba(cm.getPlayer().getName() + "『强化公告』" + " : " + "在自由相框-强化成功！大家一起恭喜他（她）吧！",9);
					cm.dispose();
				}else {
					cm.setBossLog('qianghua1');
					cm.sendOk("#r#e强化失败！,祝您游戏愉快!#k装备并未有任何变化!");	 
					//Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(9,cm.getClient().getChannel(),"『强化公告』" + " : " + " 玩家 [" + cm.getPlayer().getName() + "]在自由市场-饮水机-强化失败！大家一起安慰他(她)吧！",true).getBytes());
