...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if(cm.getJob() >= 1000 && cm.getJob() <= 1510){
			 ...
			 if(cm.getJob() >= 2000){
                cm.sendNext("哇~~战神燃起来！新职业哦~我很高兴为你服务哦！！！");
                status = 163;
                return;
			 }	
    ...
    cm.teachSkill(5221010,0,5);
            ...
            if(cm.getJob() == 2000 && cm.getLevel() >=10){
                ...
                cm.sendYesNo("战神燃起来！您真的确定要进行第二次转职了吗？");
            } else if(cm.getJob() == 2110 && cm.getLevel() >=70){
                cm.sendYesNo("战神燃起来！您真的确定要进行第三次转职了吗？");
            } else if(cm.getJob() == 2111 && cm.getLevel() >=120) {
                cm.sendYesNo("战神燃起来！您真的确定要进行第四次转职了吗？");
            ...
            if(cm.getJob() == 2000 && cm.getLevel() >=10){
				...
				cm.teachSkill(21000002,0,10);
				cm.teachSkill(21001001,0,20);
