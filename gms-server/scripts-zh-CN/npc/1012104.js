...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.sendSimple("您好！我是美容店的助手伯丽特。如果你有#b射手村美发店普通会员卡#k，就把头发交给我处理吧。怎么样？\r\n#b#L0#换发型#v5150000##z5150000#\r\n#L1#染头发#v5151000##z5151000#");
    } else if (status == 1) {
        if (selection == 0) {
            ...
            cm.sendYesNo("如果使用普通会员卡，你就不能指定发型，怎么样？你真的想用#b普通会员卡#k换发型吗？");
        } else if (selection == 1) {
            ...
            cm.sendYesNo("如果使用普通会员卡，你就不能指定你喜欢的颜色，怎么样？你真的想用#b普通会员卡#k换发型吗？");
        }
    } else if (status == 2) {
        if (beauty == 1) {
            if (cm.setRandomAvatar(5150000, hair_Colo_new) == 1) {
                ...
                cm.sendOk("嗯……你是不是没有我们美发店的专用会员卡啊？不好意思，没有会员卡的话，我就不能帮你修剪头发。");
            }
        } else {
            if (cm.setRandomAvatar(5151000, hair_Colo_new) == 1) {
                cm.sendOk("好了,让朋友们赞叹你的新发色吧!");
            } else {
                cm.sendOk("嗯… 你不会是没有我们美发店的会员卡吧！不好意思，如果没有会员卡，我们不可以给你打理头发。");
            }
        }
        cm.safeDispose();
    }
