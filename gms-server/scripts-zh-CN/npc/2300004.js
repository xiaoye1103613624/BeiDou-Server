...
var 列表 = [
	...
	{
		坐标: 2140000002, 标题: "扩充", 编辑A: "", 编辑B: "",
		介绍: [
			{ 编辑: "#k可使用点卷增加每天可挑战次数" },
		]
	},
	{
		坐标: 2140000003, 标题: "分解", 编辑A: "", 编辑B: "",
		介绍: [
			{ 编辑: "分解以下装备获得对应积分" },
		...
		//	{ 编辑: "#v1690000:##r#z1690000#类  每件[10积分以上] " },
...
var 扩充 = { 类型: "点券", 价格: 2000, 扩充Log: "维利塔斯", 上限: 1 }; //世界树扩充 改了影响到BOSS门口NPC，要同步
...
var 商城 = [
	{ 代码: 3605011, 价格: 200, 数量: 1, 类型: "积分" },
    { 代码: 3605011, 价格: 2000, 数量: 10, 类型: "积分" },
	...
	{ 代码: 3700290, 价格: 100, 数量: 1, 类型: "积分" },
    { 代码: 3700290, 价格: 1000, 数量: 10, 类型: "积分" },
	...
	{ 代码: 2340000, 价格: 1000, 数量: 1, 类型: "积分" },
    { 代码: 2340000, 价格: 10000, 数量: 10, 类型: "积分" },
	{ 代码: 2340000, 价格: 100000, 数量: 100, 类型: "积分" },
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			switch (标题) {
				case "材料": text += 购买材料A(选择1); break;
				case "扩充": text += 扩充次数A(); break;
				case "分解": text += 分解装备A(); break;
				case "商城": text += 商城购买A(选择1); break;
				default:
					break;
			}
			cm.sendYesNo(text);
			...
			switch (标题) {
				case "材料": 选择1 = 购买材料B(sele1, 选择1); start(); break;
				case "扩充": 扩充次数B(); start(); break;
				case "分解": 分解装备B(sele1); start(); break;
				case "商城": 选择1 = 商城购买B(sele1, 选择1); start(); break;
					break;
				default:
					break;
			}
...
function 购买材料B(sele1, 选择1) {
	switch (sele1) {
		case 是:
			var 筛选 = 材料.支付[选择1];
			if (!cm.canHold(材料.代码, 1)) {
				cm.getPlayer().dropMessage(5, "您的背包满咯！！！");
			} else {
				扣除代价(筛选.类型, -筛选.价格);
				cm.gainItem(材料.代码, 20);
				cm.getPlayer().setBossLog("世界树材料", 1);//每日LOG
...
function 购买材料A(选择1) {
	var text = "";
	var Log = cm.getPlayer().getBossLog("世界树材料");
	if (选择1 == null) {
		text += "\t#d" + 广播 + " 每天可以购买[#b" + Log + "#d/#r" + 材料.上限 + "#d]次,下方请选择购买方式：\r\n";
		for (var j = 0; j < 材料.支付.length; j++) {
			var 筛选 = 材料.支付[j];
			text += "#L" + j + "##d" + 箭头 + " " + 呈现代价(筛选.类型, 筛选.价格) + "购买：#v" + 材料.代码 + ":# *20个#l\r\n";
		}
	} else {
		var 筛选 = 材料.支付[选择1];
		text += "\t#d" + 广播 + " " + 呈现代价(筛选.类型, 筛选.价格) + "购买：#v" + 材料.代码 + ":#\r\n";
		if (Log >= 材料.上限) {
			text += "\t#d" + 广播 + " 每天只能购买[#b" + Log + "#d/#r" + 材料.上限 + "#d]次！当天达到上限！\r\n";
