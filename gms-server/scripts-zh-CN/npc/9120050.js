...
function start() {
    if (cm.getMapId() == 802000820) {
        ...
        if (prop == null || prop.equals("0")) {
            var exped = cm.getExpedition(ExpeditionType.AUFHEBEN);
            if (exped == null) {
                status = 0;
                cm.sendYesNo("你想成为远征队长吗？");
            } else if (cm.isLeaderExpedition(ExpeditionType.AUFHEBEN)) {
                status = 10;
...
function action(mode, type, selection) {
    ...
    case 0:
        if (mode == 1) {
            var result = cm.createExpedition(ExpeditionType.AUFHEBEN);
            if (result == 0) {
    ...
    case 5:
        ...
        var exped5 = cm.getExpedition(ExpeditionType.AUFHEBEN);
        if (exped5 == null) {
    ...
    case 10:
        ...
        var exped10 = cm.getExpedition(ExpeditionType.AUFHEBEN);
        if (exped10 == null) {
            cm.sendOk("远征队已经注销，请重新发起。");
            cm.dispose();
            break;
    ...
    case 11:
        if (mode == 1 && selection >= 0) {
            var exped11 = cm.getExpedition(ExpeditionType.AUFHEBEN);
            if (exped11 != null) {
    ...
    case 12:
        if (mode == 1 && selection >= 0) {
            var exped12 = cm.getExpedition(ExpeditionType.AUFHEBEN);
            if (exped12 != null) {
