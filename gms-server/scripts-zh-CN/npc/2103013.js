...
var fubenm = "金字塔经验副本"; //副本名称
var bossm = "维利塔斯"; //BOSS名称+大陆扩充
var bossid = 9600086; //BOSSID
var bossjyxs = "500万"; //BOSS经验显示
var bossjy = 5000000; //BOSS经验
var bossxlxs = "1滴血"; //BOSS血量显示
var bossxl = 1; //BOSS血量
var minLevel = 70; //最低等级
var maxLevel = 250; //最高等级
var minPartySize = 1; //最低人数
var maxPartySize = 2; //最高人数
var TZCS = 1; //限制次数
var cywp = 3994730; //消耗物品
var xhwzsl = 1; //消耗物品数量
var zlyqxs = "20 万"; //战力要求显示
var zlyq = 1; //战力要求
var fubendt = 926010001; //副本地图
var FBSJ = 1200; //限制时间 秒
var FBSJXS = 20; //显示时间分钟
var inmeso = 1; //入场金币

var propsToDiscard = [2022701, 2022702, 2022703, 2022704, 2022705, 2022706, 2022707, 2022708, 2022709, 2022710]; // 需要丢弃的道具ID列表
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "   \t\t\t" + 图标1 + "#d 今日次数:#r " + bossLogCount + "#b/" + TZCS + "次\r\n\r\n\r\n"; // 显示扩充后的总次数
            text += "   \t\t\t" + 图标1 + "#d 副本时间:#b " + FBSJXS + " 分钟\r\n\r\n\r\n"; // 显示扩充后的总次数
			text += "   \t\t\t" + 图标1 + "#d 怪物血量:#r " + bossxlxs + "#k\r\n\r\n";
			text += "   \t\t\t" + 图标1 + "#d 副本门票:#r #v" + cywp + "# * " + xhwzsl + " (点击商城可兑换)#k\r\n\r\n";
		//	text += "    \t\t\t" + 提示 + "#b 点击商城，可以进行门票金币兑换\r\n";
			
            if (cm.getPlayer().getMapId() == fubendt) {
				text += "\r\n    \t\t\t #L2##r" + 图标3 + "退出副本" + 图标3 + "#k#l#n\r\n\r\n";
            } else {
				text += "\r\n   \t\t\t #L1##v3800350# #r确定进入 #v3800350##l  \r\n\r\n";
            }
			
            text += "\r\n" + 群粉心 + "\r\n";
			text += "\t\t" + 提示 + " #d地图内可以使用轮回石碑增加怪物数量\r\n";
			text += "\t\t" + 提示 + " #b#z" + cywp + "#点击商城，可进行门票金币兑换\r\n";
            ...
            if (cm.getPlayerCount(fubendt) > 0) {
                ...
                cm.dispose();
                return;
			} else if (cm.getBossLog(fubenm) >= TZCS ) { // 检查扩充后的总次数
                cm.sendOk("您今天次数已经用完了，请明天再来");
                cm.dispose();
                return;
                ...
                cm.sendOk("请核对你的队伍人数！最多2人（夫妻有经验加成）");
                cm.dispose();
                return;
            } else {
        var msg = '';
        ...
        if (msg.length > 0) {
            ...
            return;
                ...
                while (it.hasNext()) {
                    ...
                    if (chr == null) {
                        ...
                        var inventory = chr.getInventory(MapleInventoryType.SETUP);  //这个ETC是背包其他栏，消耗道具使用 MapleInventoryType.USE 来检查
                        if (inventory.countById(cywp) < xhwzsl) {
                            memberCheckPassed = false;
                            memberStatusText += chr.getName() + " 没有#v" + cywp + "#或数量不足！\r\n";
                        }

                        if (chr.getBossLog(fubenm) >= TZCS ) { // 检查扩充后的总次数
                            memberCheckPassed = false;
                ...
                if (memberCheckPassed) {
                    ...
                    //cm.召唤怪物(bossid, bossxl, bossjy, 1, fubendt, 604, 227);
                   ...
                   // cm.getPlayer().startMapTimeLimitTask(FBSJ, cm.getChannelServer().getMapFactory().getMap(910000000));       //这个给个人地图记时
					...
					if (chr != null) {
					// 为每个队员启动地图时间限制任务
					chr.startMapTimeLimitTask(FBSJ, cm.getChannelServer().getMapFactory().getMap(926010000));
					}
				}

                    cm.喇叭(2, "【" + cm.getName() + "】 队伍开始挑战 【" + fubenm + "】，半个小时后再见");
                    cm.dispose();
                } else {
            ...
            // 如果玩家在副本地图，选择退出
            cm.warpParty(926010000); // 示例退出到主城地图
            cm.dispose();
        }
    }
}
