    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            //text += "#L0##b" + 蓝色小兔子 + "测试地图是否有人 \r\n"
			 text += "#e#k进入条件:#v3994731#x10个,破功99万 250级[今日次数"+cm.getBossLog("黑暗希拉")+"/3]\r\n\r\n#e#kboss暴率:#v2022511##v1003800##v1042255##v1062169##v2531000#\r\n\r\n"
            text += "#L1##b挑战黑暗希拉boss[1000E]   #d状态: " + cm.getPlayerCount(262030300) + " #l\r\n"
		 ...
		 if (selection == 1) { //挑战双头蛇  3994389
				cm.sendYesNo(""+ 任务简介 +"\r\n1.当前挑战的BOSS为#d[黑暗希拉]#k 血量 #r[1000E]#k\r\n2.当前挑战次数 #b"+cm.getPlayer().getBossLog('黑暗希拉')+"/3 #k次\r\n3.请确保背包拥有#v3994731##d(每次挑战消耗10个)#k\r\n4.#v3994731#可在#b中介兑换获取#k\r\n\r\n "+蓝色小兔子+"#e#d您确定现在进行挑战吗?#k#l");
				beauty = 1;
		...
		// ------------------------------beauty部分-------------------------------------- //
			 ...
			 if (cm.getPlayer().getParty() == null){
				...
				cm.sendOk(""+ 任务简介 +"\r\n您不是队长,请让队长与我对话#k");
				cm.dispose();
				}else if (cm.getPlayer().getVipczz()<79){
				cm.sendOk(""+ 任务简介 +"\r\n您的破功不足99万#k");
				cm.dispose();
			}else if(!cm.haveItem(3994731,10)){
				cm.sendOk(""+ 任务简介 +"\r\n1.当前挑战的BOSS为#d[黑暗希拉#k 血量 #r[1000E]#k\r\n2.当前挑战次数 #b"+cm.getPlayer().getBossLog('黑暗希拉')+" #k次 #d[每日每个账号可挑战3次]#k\r\n3.请确保背包拥有#v3994731##d(每次挑战消耗10张)#k\r\n\r\n");
				cm.dispose();
				}else if(cm.getPlayer().getLevel() <= 249 ){
				cm.sendOk(""+ 任务简介 +"\r\n1.当前挑战的BOSS为#d[黑暗希拉#k 血量 #r[1000E]#k\r\n2.当前挑战次数 #b"+cm.getPlayer().getBossLog('黑暗希拉')+" #k次 #d[每日每个账号可挑战3次]#k\r\n3.请确保背包拥有#v3994731##d(每次挑战消耗10张)#k\r\n\r\n");
				cm.dispose();
				}else if(cm.getBossLog('黑暗希拉') >= 3){
				cm.sendOk(""+ 任务简介 +"\r\n1.当前挑战的BOSS为#d[黑暗希拉#k 血量 #r[1000E]#k\r\n2.当前挑战次数 #b"+cm.getPlayer().getBossLog('黑暗希拉')+" #k次 #d[每日每个账号可挑战3次]#k\r\n3.请确保背包拥有#v3994731##d(每次挑战消耗10张)#k\r\n\r\n");
				cm.dispose();
			}else if (cm.getPartyBossLog('黑暗希拉',3)== false){
				cm.sendOk(""+ 任务简介 +"\r\n您的组队有人挑战过3次了#k");
				cm.dispose();
			
			}else if(cm.getPlayerCount(262030300) >= 1){
				cm.sendOk(""+ 任务提示 +"\r\n当前频道BOSS地图可能已有人挑战,请切换频道");
				cm.dispose();
			}else{
				cm.warpParty(262030300,0); // 传送到BOSS地图
				cm.gainItem(3994731,-10);
				cm.刷新地图();
				// 召唤怪物 怪物id 地图id ,坐标
				// cm.spawnMob_map(9601015,745010300,1116,337);//xml实现血量
				// cm.spawnMob_map(怪物ID,数量,x坐标,y坐标,地图id,血量); 
				cm.spawnMobOnMap(9800202,1,156,196,262030300,100000000000); // npc实现血量	
				cm.givePartyBossLog('黑暗希拉');
