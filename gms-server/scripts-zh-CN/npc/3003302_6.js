		...
		//	var 爱心 = "#fEffect/CharacterEff/1022223/4/0#";
			...
			if (!cm.haveItem(3992010,100)) {
				...
				if (sj == 0) {
					...
					//Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(9,cm.getClient().getChannel(),"[宠物系统]" + " : " + cm.getPlayer().getName() +"强化了宠物,敏捷 [+"+lvsj+"] ！！",true).getBytes());
           
					//cm.getClient().getChannelServer().getWorldInterface().broadcastMessage(null,MaplePacketCreator.getItemMega(cm.getClient().getChannel(),cm.getPlayer().getName() + " : " + "在自由市场-强化NPC-强化成功！大家一起恭喜他（她）吧！！！",item, true).getBytes());
