...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
	    if (cm.getQuestStatus(6192) == 2) {
				cm.sendOk("该干嘛就干嘛去！");
                cm.dispose();
                return;			
		}	

	    if (cm.getQuestStatus(6192) == 8) {
			cm.forceCompleteQuest(6192);//强制完成任务
		if (cm.getParty() != null) {
		...
		cm.forceCompleteQuest(6192);//强制完成任务
		cm.playerMessage(5, "恭喜你！完成任务！已经获取技能了哦！");
		//cm.sendOk("你保护了我。谢谢。我已经给了你稳如泰山！！！");
		return;
		}
	    ...
	    return;
			...
			if (cm.haveItem(4031057, 1)){
                cm.sendNext("恭喜你到达这里,最后我将给你一个考验!");//211040401			
            ...
            if (cm.haveItem(4031058, 1)) {
                if (cm.getJob()==110) {
					...
					cm.sendOk("恭喜转职了!");
					cm.worldMessage("‘转职快报’：恭喜玩家."+ cm.getChar().getName() +"  成功三转-勇士让我们热烈的祝福他/她吧！");
                    cm.dispose();
					...
					cm.sendOk("恭喜转职了!");
					cm.worldMessage("‘转职快报’：恭喜玩家."+ cm.getChar().getName() +"  成功战神三转让我们热烈的祝福他/她吧！");
                    cm.dispose();
				}
            } else if (cm.haveItem(4031057, 1))
