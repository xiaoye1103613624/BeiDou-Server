...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		cm.sendNext("The Energy Conducting Device was connected to an egg. While I was examining it, that person broke out of the egg and defeated all of the Black Wings. It was crazy!", 2159342);
	} else if (status == 11) {
		cm.sendNext("If it was anyone else telling me this, I would laugh. But this... who is this person?", 2159315);
	} else if (status == 12) {
		cm.sendPlayerToNpc("They're talking about me...");
	} else if (status == 13) {
		cm.sendNext("I've never seen skills like his. So powerful... If he hadn't used up all his power, I wouldn't have brought him here. He's dangerous. Is he a test subject? We must stop that blasted mad man Gelimer.\r\nIn any case, he should be up by now. I'll check on him.", 2159342);
	} else if (status == 14) {
		cm.sendDirectionStatus(3, 2);
		cm.sendDirectionStatus(1, 2000);
		cm.sendNext("Ah, you're awake. How do you feel? Tired?", 2159342);
	} else if (status == 15) {
		cm.sendPlayerToNpc("Did you save me?");
	} else if (status == 16) {
		cm.sendNext("Yes, you were badly wounded. I can't just leave you there with the Black Wings. I think we're on the same side. We have plenty to talk about, so how about you come with me?", 2159342);
	} else if (status == 17) {
