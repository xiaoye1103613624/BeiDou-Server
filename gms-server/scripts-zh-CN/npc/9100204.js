...
function action(mode, type, selection) {
    ...
    if (status == 0) {
    	var txt = "#d使用枫叶进行抽奖，您将随机获得下面的奖励#n#k\r\n";
		txt += "#d当前枫叶剩余：#b#c4001126#\r\n";
		txt += "#d每次抽奖消耗：#b100枫叶\r\n\r\n";
    	...
    	switch (sel0) {
			case 1:
                if (!cm.haveItem(4001126,100*selection)) 
				{
					cm.sendOk("#v4001126#数量不足#r "+(100*selection)+" #k，无法抽奖");
					cm.dispose();
					return;
				} 
				else 
				{
					...
					{
						cm.gainItem(4001126, -(100*selection));
					}
				}
				break;
			case 2:
		...
		for (var i = 0; i < selection; i++) {
			...
			else
			{
				...
				cm.gainGachaponItem2(ran1[0], ran1[2], "枫叶抽奖", 1);
			}
