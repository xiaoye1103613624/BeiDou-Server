...
function action(mode, type, selection) {
    ...
    if (status == 0) {
			...
			//cm.gainItem(4001158, 1);//飞天蛋
			//cm.gainGP(+20);//给家族GP点
			//cm.喇叭(4,"[组队副本]"," 玩家:<"+cm.getName()+">完成女神塔组队副本,大家恭喜吧！！！");
	        //cm.gainAp(2);
			//cm.getPlayer().setBossLog("天空组队",0);
			/*if (cm.getPlayer().getBossLog("天空组队",0) == 6){
				//cm.gainItem(4001010,1);
				cm.喇叭(4,"[组队副本]"," 玩家:<"+cm.getName()+">完成6次女神塔组队副本,获得[蘑菇王橡皮擦]！！！");
			}*/
			if(cm.isLeader()){
			cm.喇叭(1,"[组队副本]玩家:<"+cm.getName()+">完成女神塔组队副本,大家恭喜吧！！！");
				}
            cm.dispose();
