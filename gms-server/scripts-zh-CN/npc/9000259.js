	...
	else 
	{
        ...
        if (status == 0) 
		{
			...
			if(cm.getPlayer().getMapId()==180000001)
			{
				...
				add +=" "+蝙蝠魔+光球+怪物+"\r\n\r\n"
				//add += "         #r[ #d所有BOSS均会掉落材料和不同的装备 #r]\r\n";
				//add += "               #r[ #d目前已加入#r247#d件新装备 #r]\r\n";
				add += "           #r[ #d称号BOSS掉落称号材料和不同的装备 #r]\r\n";
				add += "           #r[ #d破攻BOSS掉落破攻材料和不同的装备 #r]\r\n";
				add += "           #r[ #d战力BOSS掉落高级装备和不同的装备 #r]\r\n";
				add += "\r\n\r\n"
				//add += "\t          #L1#"+红色箭头+蓝色箭头+"  #k挑战钻机（战力）#l \r\n";
				add += "\t          #L2#"+红色箭头+蓝色箭头+"  #b挑战战舰（破攻）#l\r\n";
				//add += "\t          #L3#"+红色箭头+蓝色箭头+"  #r挑战贝尔（称号）#l\r\n\r\n";
...
//------------------------------------------------------------------------
		...
		else if (status == 1) 
		{
			...
			{
					cm.dispose();
					cm.openNpc(9000434, "挑战钻机");
			}
			if (selection == 2) 
			{
					cm.dispose();
					cm.openNpc(9000434, "挑战战舰");
			}
			if (selection == 3) 
			{
					cm.dispose();
					cm.openNpc(9000434, "挑战贝尔");
			}
		}
	}
}
