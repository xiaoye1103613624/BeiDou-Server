...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {//#L2##e5000点券=====#r提升1级破攻等级(10W攻击上限)#k#n#l#k
            cm.sendSimple ("您好，尊敬的 #b#h ##k, 我是" + cm.getChannelServer().getServerName() + "的#r系统功能#k负责人\r\n每增加一级破功等级，破功上限增加10W\r\n\r\n您目前破攻等级为： #e#r" + cm.getvip() + "#k#n \r\n\r\n#r#e抱歉GM目前尚未开放此功能!\r\n\r\n");
        } else if (status == 1) {
            switch(selection) {
        case 2:
            if(cm.getNX(1) >= 5000){
                cm.sendOk("恭喜你，你提升了1级破功等级! .");
				cm.gainvip(+1);
                cm.gainNX(-5000);
				cm.worldMessage(6,"【破攻系统】["+cm.getName()+"]提升了1级破功等级!");
                cm.dispose();
            }else{
                cm.sendOk("你没有 足够的点券，我不能给你提升~.");
                cm.dispose();
            }
            break;
        case 1: 
