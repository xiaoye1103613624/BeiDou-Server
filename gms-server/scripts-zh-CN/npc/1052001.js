    ...
    if (status == 0) {
        ...
        if (cm.getJob() == 0) {
            if (cm.getPlayer().getLevel() >= 10) {
                cm.sendNext("你要轉職成為一位 #r飞侠#k ?");
            } else {
                cm.sendOk("你還不能轉職成為 #r飞侠#k。");
                cm.dispose();
            }
        } else {
            ...
            } else if (cm.getPlayer().getLevel() >= 70 && cm.getJob() == 410 || cm.getJob() == 420 || cm.getJob() == 432) {
                if (cm.canHoldByType(4, 2)) {
                    if (cm.haveItem(4031059, 1)) {
                        cm.gainItem(4031057, 1);
                        cm.gainItem(4031059, -1);
                        cm.warp(211000001, 0);
                        cm.sendOk("你完成了一個考驗，現在去找 #b艾瑞克#k。");
                    } else {
                        cm.sendOk("嗨, #b#h0##k! 我需要一个 #b黑符#k。快去#r猴子沼泽地Ⅱ#k的#b异界之门#k里拿給我。");
                    }
                } else {
        ...
        cm.sendNextPrev("你可以選擇你要轉職成為一位 #r刺客#k, #r侠客#k。")
    } else if (status == 12) {
        cm.askAcceptDecline("但是我必須先測試你,你準備好了嗎？");
    } else if (status == 13) {
        cm.gainItem(4031011, 1);
        cm.warp(102040000);
        cm.sendOk("請去找 #b飞侠轉職教官#k。他會幫助你的。");
        cm.dispose();
    } else if (status == 21) {
