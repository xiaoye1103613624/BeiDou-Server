    ...
    if (mode == -1) {
		...
		if (status == 0) {
			...
			if(sel_type == 0){
				...
				var selStr = "#d固有道具每人只能拥有一件，包括身上穿的，请谨慎操作！\r\n\r\n#b#L0##e打开全部商品信息#n#l\r\n";
				var iz = mm.getOnlygoods_list().iterator();
				var list_num = 0;
				...
				// cm.sendNext("#b正在为您查询数据 >>>>");
				var selStr = "#d固有道具每人只能拥有一件，包括身上穿的，请谨慎操作！\r\n\r\n#b#L0##e打开全部装备信息#n#l\r\n";
				var iz = mm.getOnlyeq_list().iterator();
				var list_num = 0;
				for(var i = 0; iz.hasNext(); i++){
					var zz = iz.next();
					selStr += "#L"+zz+"##b#i"+zz+":##l ";
				...
				//selStr += "\t\t#r#L666#           收益兑换元宝#l";
			    selStr += "\r\n\r\n "+美化4+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化6+""+美化5+"";
				cm.sendSimple(selStr);
			} else if (sel_type == 3){
				var selStr = "#d目前等待上架物品！\r\n\r\n#b";
				var iz = mm.getGoods_list().iterator();
				var list_num = 0;
				for(var i = 0; iz.hasNext(); i++){
					var zz = iz.next();
					if((zz.getGood_num() <= 0) || (zz.getCreateData() + 1000*60*60*1  <= new Date().getTime() || zz.getGood_num() <= 0)){
						continue;
					}
					selStr += "#b#i"+zz.getGood_id()+":# 数量：#r#e"+printf(zz.getGood_num(), 3)+"#n#b 售价：#r#e"+printf(zz.getGood_price(), 3)+"#n#b 商人：#r#e"+zz.getCharacters_name()+"#n#b \r\n\t\t倒计时:#r#e"+ parseInt((zz.getCreateData() + 1000*60*60*1 - new Date().getTime())/(1000*60*60)) +"时"+ parseInt((zz.getCreateData() + 1000*60*60*1 - new Date().getTime() - parseInt((zz.getCreateData() + 1000*60*60*1 - new Date().getTime())/(1000*60*60))*(1000*60*60))/(1000*60)) +"分#n#b\r\n";
					selStr += "\r\n";
					list_num++;
				}
				cm.sendOk(selStr);
				...
				var selStr = "#d目前等待上架物品！\r\n\r\n#b";
				var iz = mm.getEqs_list().iterator();
				var list_num = 0;
				for(var i = 0; iz.hasNext(); i++){
					var zz = iz.next();
					if((zz.getGood_num() <= 0) || (zz.getCreateData() + 1000*60*60*1  <= new Date().getTime())){
						continue;
					}
					selStr += "#b#i"+zz.getGood_id()+":# 售价：#r#e"+printf(zz.getGood_price(), 3)+"#n#b 商人：#r#e"+zz.getCharacters_name()+"#n#b 倒计时:#r#e"+ parseInt((zz.getCreateData() + 1000*60*60*1 - new Date().getTime())/(1000*60*60)) +"时"+ parseInt((zz.getCreateData() + 1000*60*60*1 - new Date().getTime() - parseInt((zz.getCreateData() + 1000*60*60*1 - new Date().getTime())/(1000*60*60))*(1000*60*60))/(1000*60)) +"分#n#b\r\n"+zz+"\r\n";
					selStr += "\r\n";
					list_num++;
				}
				cm.sendOk(selStr);
		...
		//} else if (sel_type == 666) {
			...
			if(sel_type == 0 ){
				...
				for(var i = 0; iz.hasNext(); i++){
					var zz = iz.next();
					if(zz.getCreateData() + 1000*60*60*1  >= new Date().getTime() || zz.getGood_num() <= 0 || (selection != 0 && zz.getGood_id() != selection)){
						continue;
					}
					selStr += "#L"+i+"##b#i"+zz.getGood_id()+":# 剩余数量：#r#e"+printf(zz.getGood_num(), 3)+"#n#b 售价：#r#e"+printf(zz.getGood_price(), 3)+"#n#b 商人：#r#e"+zz.getCharacters_name()+"#n#b#l\r\n";
					selStr += "\r\n";
				...
				for(var i = 0; iz.hasNext(); i++){
					var zz = iz.next();
					if(zz.getCreateData() + 1000*60*60*1  >= new Date().getTime() || (selection != 0 && zz.getGood_id() != selection)){
						continue;
					}
				...
				if(sr_type == 0){
					...
					for (var j = 2; j <= 5; j++) {
						for (var i = 0; i < 96; i++) {
							...
							if (!MapleItemInformationProvider.getInstance().isQuestItem(cm.getInventory(j).getItem(i).getItemId())) {
					...
					cm.sendGetNumber("#d请输入你要兑换元宝的数量：\r\n#b比列：1：1",0,1, cm.getmoney());
				} else if(sr_type == 666){
					cm.sendNext("请联系群主或管理提取元宝，谢谢（满100起提并收取5%手续费）。");
					status = -1;
				}
			} else if(sel_type == 2 ){
				var selStr = "#d目前交易市场商品列表！\r\n\r\n#b";
			...
			if(sel_type == 0){
				select_good = mm.getGoods_list().get(selection);
				cm.sendGetNumber("#d当前你选择物品信息：\r\n\t#b#i"+select_good.getGood_id()+":##z"+select_good.getGood_id()+":# 商人：#r#e"+select_good.getCharacters_name()+"#n#b\r\n\t剩余数量：#r#e"+select_good.getGood_num()+"#n#b\t当前售价：#r#e"+select_good.getGood_price()+"#n#b\r\n目前你有 #b元宝#k × #r#e"+getmoneyb()+"#n#k\r\n#r#e请输入你要购买的数量：#n#k",0,1,parseInt(getmoneyb()/select_good.getGood_price()) > select_good.getGood_num() ? select_good.getGood_num() : parseInt(getmoneyb()/select_good.getGood_price()));
			} else if(sel_type == 2){
				select_good = mm.getEqs_list().get(selection);
				cm.sendYesNo("#d当前你选择物品信息：\r\n\t#b#i"+select_good.getGood_id()+":##z"+select_good.getGood_id()+":# 商人：#r#e"+select_good.getCharacters_name()+"#n#b\r\n\t"+select_good+"\r\n\t当前售价：#r#e"+select_good.getGood_price()+"#n#b\r\n目前你有 #b元宝#k × #r#e"+getmoneyb()+"#n#k\r\n#r#e是否要继续购买#n#k");
			} else if(sel_type == 1){
				if(sr_type == 0){
					if(cm.getmoneyb() > -1){
						if(cm.haveItem(5140005, 0)){
							cm.gainItem(5140005, 1, 30*24);
							cm.setmoneyb(-消费元宝);
							
							cm.sendOk("#r#e注册成功!在卖货的道路上祝您一帆风顺！#n#b\r\n\r\n#i5140005:##z5140005#这个是您的商人凭证我帮您放在背包[设置栏]内,请不要搞丢了哦!");
							//cm.喇叭(5390005, "商人入驻", "玩家 "+cm.getPlayer().getName()+" 注册了商人,祝您在卖货路上一帆风顺!");
							cm.dispose();
						} else {
					...
					if(cm.getmoney() >= selection){
						cm.setnmoney(-selection);
						cm.setmoneyb(parseInt(selection));
			...
			if(sel_type == 0){
				if(selection <= select_good.getGood_num()){
					...
					if(getmoneyb() >= totel_price){
						//if(cm.haveItem(select_good.getGood_id(), selection)){
							if(select_good.gainGood_num(-selection)){
								cm.set单笔充值(parseInt(-totel_price));
								gainmoneyb(select_good.getAcc_id(), parseInt(totel_price*1));
				...
				if(selection <= select_good.getGood_num()){
					...
					if(getmoneyb() >= totel_price){
						...
						/*} else {
							cm.sendNext("#b操作已被驳回,原因：背包不能装下物品11");
						}*/
					} else {
						cm.sendNext("#b很抱歉,你没有这么多元宝!");
					}
				...
				if(sr_type == 1){
					...
					if(selection <= select_good.getGood_num()){
						...
						if(!cm.haveItem(select_good.getGood_id(), selection)){
							...
							cm.sendNext("#b操作已被驳回,原因：背包已经有此物品");
						}
					} else {
			...
			if(sel_type == 1){
				if(sr_type == 1){
					if(cm.haveItem(sj_itemid, sj_num)){
						...
						mm.add_good(cm.getPlayer(), sj_itemid, sj_num, sj_price, new Date().getTime());
						cm.sendOk("#r#e物品上架成功~!期待爆单！！1小时后可以开放出售!");
