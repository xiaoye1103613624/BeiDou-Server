...
function start() {
    ...
    if (MapObj == null) {
        ...
        MapObj = cm.getMap();
        // 获取当前地图存活的怪物数量，由于未找到获取当前地图固定怪物列表方法，故用此方法代替。
        List_Mob_All = MapObj.getAllMonsters();
        //将怪物种类去重并按照Boss和普通怪物区分开
        [List_Mob, List_Mob_Boss] = Object.values(List_Mob_All.reduce((acc, mob) => (acc.ids.has(mob.getId()) || (acc.ids.add(mob.getId()), mob.isBoss() ? acc.bosses : acc.mobs).push(mob), acc), {
            ids: new Set(),
            mobs: [],
            bosses: []
        })).slice(-2);
    ...
    if (mob != null) {
        ...
        if (CountItems <= 0) {
            ...
            Object.keys(table).forEach(key => {
                table[key] = Math.max(table[key], key.length)
            });
            let dropitemlist = {};
