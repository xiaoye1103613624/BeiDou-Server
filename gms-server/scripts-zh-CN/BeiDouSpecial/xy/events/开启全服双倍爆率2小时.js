...
**/
// ==================== 配置变量 ====================
var 扣除点券数量 = 1;          // 开启双倍掉落需要扣除的点券数量（nxCredit）
var 开双时限小时 = 2;          // 双倍掉落持续时间（小时）
var 事件名称 = "2xDropEvent";  // 服务端事件脚本名称
    ...
    if (mode == -1) {
        ...
        if (status == 1) {
            if (selection == 1) {
                ...
                // 保存原始掉落倍率，以便结束后恢复
                var originalDropRate = cm.getPlayer().getWorldServer().getDropRate();
                ...
                cm.getPlayer().getWorldServer().broadcastPacket(
                    PacketCreator.serverNotice(5, "【双倍掉落】恭喜 [" + cm.getName() + "] 为全服开启了双倍掉落活动，持续" + 开双时限小时 + "小时，要刷装备的赶紧了！")
                );
