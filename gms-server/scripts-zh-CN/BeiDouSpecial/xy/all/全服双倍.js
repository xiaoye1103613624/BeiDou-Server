/*
	自助开双脚本 —— 全服双倍经验
	玩家可通过消耗点券为当前大区开启双倍经验
	全服生效，不可叠加，不可重复开启
**/
// ==================== 配置变量 ====================
var 扣除点券数量 = 100000;        // 开启双倍需要扣除的点券数量（nxCredit）
var 开双时限小时 = 2;             // 双倍经验持续时间（小时）
var 事件名称 = "2xEvent";         // 服务端事件脚本名称（scripts-zh-CN/event/2xEvent.js）
    ...
    if (mode == -1) {
        ...
        if (status == 1) {
            if (selection == 1) {
                ...
                // 保存原始经验倍率，以便结束后恢复
                var originalExpRate = em.getWorldServer().getExpRate();
                ...
                em.getWorldServer().broadcastPacket(
                    PacketCreator.serverNotice(BroadcastPrefix.DOUBLE_EVENT.getType(), BroadcastPrefix.DOUBLE_EVENT.getPrefix() + "恭喜 [" + cm.getName() + "] 为全服开启了双倍经验活动，持续" + 开双时限小时 + "小时，要升级的赶紧了！")
                );
