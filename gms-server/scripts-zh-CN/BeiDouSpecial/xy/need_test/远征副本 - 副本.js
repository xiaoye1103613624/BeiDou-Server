...
var 挑战列表 = [
	...
	{
		事件名称: "狂暴威尔", 等级限制: 200,
		挑战记录: "狂暴威尔远征队", 挑战限制: 2, 扩充Log: "远征挑战扩充", 重返金币费用: 100000, 触发奖项怪物: 8880302,
		入口地点: 555000200, 返回地点: 910001000, 挑战地点: 555000201, 挑战时限: 60, 
		条件: [[3994789, 5]],
		增加血量: 10,//每多1个人头增加百分比血量
		怪物列表: [
			{ 代码: 8880302, 数量: 1, 血量: 640000000000, x轴: [-887, -887], y轴: 90 },
		],
		公共奖项: [],
	},
	...
	
	{
		...
		增加血量: 10,//每多1个人头增加百分比血量
		怪物列表: [
			{ 代码: 9410224, 数量: 1, 血量: 20000000000000, x轴: [-24, -24], y轴: 98 },
		],
		公共奖项: [],
	},
	
	{
		...
		增加血量: 10,//每多1个人头增加百分比血量
		怪物列表: [
			{ 代码: 8645009, 数量: 1, 血量: 40000000000000, x轴: [-252, -252], y轴: 156 },
		],
		公共奖项: [],
	},
	
	{
		...
		增加血量: 10,//每多1个人头增加百分比血量
		怪物列表: [
			{ 代码: 8880404, 数量: 1, 血量: 100000000000000, x轴: [-163, -163], y轴: 154 },
		],
		公共奖项: [],
	},
	...
	{
		事件名称: "调和精灵", 等级限制: 200,
		挑战记录: "调和精灵远征队", 挑战限制: 2, 扩充Log: "远征挑战扩充", 重返金币费用: 100000, 触发奖项怪物: 8880302,
		入口地点: 910142100, 返回地点: 910001000, 挑战地点: 910142110, 挑战时限: 60, 
		条件: [[3994789, 20]],
		增加血量: 20,//每多1个人头增加百分比血量
		怪物列表: [
			{ 代码: 8644011, 数量: 1, 血量: 300000000000000, x轴: [182, 182], y轴: -23 },
		],
		公共奖项: [],
        ...
        case 入场:
            ...
            if (em == null) {
                ...
                if (parseInt(prop) == 0 || prop == null) {
                    var map = cm.getMapFactory().getMap(列表.入口地点);
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        if (chr.getLevel() < 列表.等级限制) {
                            ...
                            // 检查玩家是否在队伍中
                            var isInParty = false;
                            var partyMembers = cm.getParty().getMembers();
                            for (var m = 0; m < partyMembers.length; m++) {
                                if (partyMembers[m].getId() == chr.getId()) {
                                    isInParty = true;
                                    break;
                                }
                            }
                            ...
                            if (isInParty) {
                                ...
                                不足人员.push({ name: chr.getName(), reason: "不在队伍中" });
                            }
                        }
                    }
...
function 读取地图人员信息(Log, limit, mapid, cost) {//
	...
	var list = map.getCharactersThreadsafe();
	text += "\t" + 美化(10, "次数") + "" + 美化(24, "玩家") + "" + 美化(15, "资质") + "\r\n";
	if (map.getCharactersSize() >= 1 && list != null) {
		for (var i = 0; i < list.length; i++) {
			...
			var 次数 = "" + 美化(18, "（#b" + chr.getBossLog(Log) + "#d/#r" + 上限次数 + "#d）") + "";
			//var 战力 = "" + 美化(11, "" + chr.getlpjf()) + "";
