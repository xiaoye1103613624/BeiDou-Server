...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		if (cm.getPlayer().getBossLog("麒麟打工系统_总次") > 0 ) {
		...
		阶段 = cm.getPlayer().getBossLog("麒麟打工系统_阶段");
		完成总次数 = cm.getPlayer().getBossLog("麒麟打工系统_总次");
		
		
		
		
		if (cm.getPlayer().getBossLog("麒麟打工系统_随机物品") == 0) {
		...
		if (selection == 1) {			
			...
			if (item.getItemId() != 需要道具){
				...
				cm.getPlayer().setBossLog("麒麟打工系统_阶段");
				...
				if (cm.getPlayer().getBossLog("麒麟打工系统_阶段",1) >= qilin.总阶段) {
					...
					cm.getPlayer().setBossLog("麒麟打工系统_总次");
