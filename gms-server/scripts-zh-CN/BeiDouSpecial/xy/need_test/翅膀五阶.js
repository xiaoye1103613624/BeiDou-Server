...
var itemObjS = {
	"1102039": [[4003005, 3000],[4003004, 3000],[4001006, 100],[4032056, 50], [4170007, 20],['金币', 200000000], ['点券', 100000]],
	"1102709": [[4003005, 3000],[4003004, 3000],[4001006, 100],[4032056, 50], [4170007, 20],['金币', 200000000], ['点券', 100000]],
	"1102605": [[4003005, 3000],[4003004, 3000],[4001006, 100],[4032056, 50], [4170007, 20],['金币', 200000000], ['点券', 100000]],
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var text = "你想拥有翅膀吗???听说在本岛帅气的人都拥有属于自己的翅膀哦~!\r\n";
            text += "当前翅膀等阶:[#r翅膀四阶#k]\r\n";
            text += "请选择你需要制作点初级翅膀[#r全属性+50#k]\r\n";
		for(var oKey in itemObjS) {
			text += "#L"+ oKey +"#兑换#v"+ oKey +"##b说明:#k[#r全属性+50#k]#k\r\n";
		}
        cm.sendSimple(text);
    } else if (status == 1) {
		...
		selected = selection;
		var text = "听说在本岛上帅气的人都会拥有属于自己的翅膀哟^!只需要以下材料就可以制作哟#k\r\n";
		text += "需求如下：\r\n";
		...
		if(!cm.haveItem(cm.getBossRank("翅膀",1), 1)) {
				cm.sendOk("请把一阶翅膀放入背包！");
				cm.dispose();
				return;	
		}
		var msg = "";
		...
		cm.gainItem(cm.getBossRank("翅膀",1), -1);
		cm.setBossRankPoints("翅膀",-cm.getBossRank("翅膀",1));
		cm.setBossRankPoints("翅膀",selected);
		cm.setBossRankPoints("翅膀等阶",1);
		cm.给属性装备(selected,0,0,50,50,50,50,0,0,50,50,0,0,0,0,0,0,0);
		cm.sendOk("进阶 #v"+ selected +"##b#z"+ selected +"##k成功。");
		cm.getPlayer().指定喇叭("高质地喇叭", "系统公告", "恭喜[" + cm.getPlayer().getName() + "]成功完成翅膀5阶!");
		cm.dispose();
	}
}