...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        // 扣除兑换道具
        cm.gainItem(EXCHANGE_ITEM.id, -EXCHANGE_ITEM.count); // 负数表示扣除
        
        // 发放180天时效皮肤
        cm.getPlayer().gainEquip(ComicList[selection][0], 10, 10, 10, 10, 200, 200, 5, 5, 5, 5, 5, 5, 0, 0, 0, 0, 180 * 60 * 24);
        
        // 领取成功提示添加道具图标
        cm.sendOk("已扣除#b" + EXCHANGE_ITEM.count + "个#v" + EXCHANGE_ITEM.id + "# " + EXCHANGE_ITEM.name + "#k！\r\n你可以享受180天的#b" + ComicList[selection][1] + "#k皮肤！");
        cm.dispose();
    }
}