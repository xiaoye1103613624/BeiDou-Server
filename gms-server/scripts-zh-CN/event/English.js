...
function setup(eim, leaderid) {
    ...
    eim.setInstanceMap(541020800).resetFully();//刷新地图
//    eim.schedule("checkStart", 1200000); // 20 min
    eim.startEventTimer(3600000); //50 minutes
    return eim;
}

function playerEntry(eim, player) {
    var NowMapID = eim.getProperty("NowMapID");
    var toMapid = 541020800;//队员进去的地图
    if (NowMapID != null) {
...
function end(eim) {
    eim.disposeIfPlayerBelow(100, 541020700);//时间到了传送出去的地图
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
