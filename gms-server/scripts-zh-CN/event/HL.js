...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("4sha_NVHUANG");

    eim.setProperty("zakSummoned", "0");
    eim.setInstanceMap(240080300).resetFully();//刷新地图
//    eim.schedule("checkStart", 1200000); // 20 min
    eim.startEventTimer(14400000); //50 minutes
    return eim;
}

function playerEntry(eim, player) {
    var NowMapID = eim.getProperty("NowMapID");
    var toMapid = 240080300;//队员进去的地图
    if (NowMapID != null) {
...
function end(eim) {
    eim.disposeIfPlayerBelow(100, 240080300);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
