...
// 修复核心问题：monsterKilled 函数增加空值判断
function monsterKilled(eim, chr, cp) {
    // 1. 检查 chr 是否为空
    if (chr == null) {
        eim.broadcastPlayerMsg(5, "击杀怪物的玩家对象为空！");
        return;
    }
    
    // 2. 检查嘉年华队伍是否为空
    var carnivalParty = chr.getCarnivalParty();
    if (carnivalParty == null) {
        eim.broadcastPlayerMsg(5, chr.getName() + " 没有加入嘉年华队伍！");
        return;
    }
    
    // 3. 安全地添加CP值
    carnivalParty.addCP(chr, cp);
    
    // 4. 检查CP相关数值获取是否安全
    try {
        chr.CPUpdate(false, chr.getAvailableCP(), chr.getTotalCP(), 0);
    } catch (err) {
        eim.broadcastPlayerMsg(5, "更新" + chr.getName() + "的CP值时出错: " + err);
        return;
    }
    ...
    while (iter.hasNext()) {
        var player = iter.next();
        if (player != null && player.getCarnivalParty() != null) {
            try {
                player.CPUpdate(true, carnivalParty.getAvailableCP(), carnivalParty.getTotalCP(), carnivalParty.getTeam());
            } catch (err) {
                eim.broadcastPlayerMsg(5, "更新" + player.getName() + "的CP显示时出错: " + err);
            }
        }
    }
}

// 注意：这里有重复定义的 monsterValue 函数，保留一个即可
function monsterValue(eim, mobId) {
    return 0;
}
...
function warpOut(eim) {
    if (!eim.getProperty("started").equals("true")) {
        ...
        if (blueParty == null || redParty == null) {
            eim.broadcastPlayerMsg(5, "红蓝队伍中有队伍为空，无法判定胜负！");
            disposeAll(eim);
            return;
        }
...
function scheduledTimeout(eim) {
    ...
    if (!eim.getProperty("started").equals("true")) {
        ...
        if (blueParty != null && redParty != null) {
            ...
            eim.broadcastPlayerMsg(5, "无法获取队伍信息，结束嘉年华");
            disposeAll(eim);
            return;
        }
...
function playerRevive(eim, player) {
    ...
    while (iter.hasNext()) {
        var p = iter.next();
        if (p != null && p.getCarnivalParty() != null) {
            p.CPUpdate(true, player.getCarnivalParty().getAvailableCP(), player.getCarnivalParty().getTotalCP(), player.getCarnivalParty().getTeam());
        }
    }
    player.addHP(50);
...
function onMapLoad(eim, chr) {
    if (!eim.getProperty("started").equals("true")) {
        disposeAll(eim);
    } else if (chr != null && chr.getCarnivalParty() != null) {
