...
function start(mode, type, selection) {
	...
	if (status == 0) {
	  ...
	  qm.sendNextPrev("Ahoy! I wanted to welcome the new knight in person. I must give ye my apologies, for I left the muffins I baked ye on my ship.", 1,0,1101007);
	} else if (status == 4) {
	  qm.sendNextPrev("Hawkeye, this is highly unorthodox!");
	} else if (status == 5) {
	  qm.sendNextPrev("Yar, have a heart, Kinu. Don't ye remember yer first day with the knights? Wasn't it a mite overwhelming without a friend?", 1,0,1101007);
	} else if (status == 6) {
...
if (mode == -1) {
	...
	if (status == 0) {
	    ...
	    qm.sendNextPrev("How kind! Thank you!", 3);
	} else if (status == 2) {
	    ...
	    qm.sendNextPrev("Hawkeye, the new recruit's name is #h #! Now, shoo!", 1,0,1102006);
	} else if (status == 4) {
	    qm.sendNextPrev("I promise, I won't forget yer name the next time we meet.");
	} else if (status == 5) {
	    qm.sendNextPrev("Finally! Where was I? Oh yes, once your initial training is over, you will choose your knightly path and be promoted to\r\nKinght-in-Training! The paths you can pick from are Light, Fire, Wind, Lightning, and Darkness.", 1,0,1102006);
	} else if (status == 6) {
	    qm.sendNextPrev("As a Cygnus Knight, your duty will be to protect the Empress, defeat the evil Black Mage, spy on his henchmen, and keep peace in Maple World. Simple, yes?", 1,0,1102006);	
	} else if (status == 7) {
	    qm.sendNextPrev("I've given you enough to ponder. Go speak with Kimu for your next lesson.", 1,0,1102006);	
    } else if (status == 8) {
