...
function start(mode, type, selection) {
	...
	if (status == 0) {
        ...
        qm.sendNextPrev("A new... #bExplorer?#k What's that?",17);	
	} else if (status == 2) {	
        qm.sendNextPrev("This was on that test I flunked ten times... Oh, right! Explorers are people who come to Maple World from other worlds! They start their journey right here on #bMaple Island#k.",1);		
    } else if (status == 3) {	
	    qm.sendNextPrev("I'm on... #bMaple Island?#k",17);	
	} else if (status == 4) {	
	    qm.sendNextPrev("You sure are! We used to be just some tiny island, but then Explorers started popping out. Now we've even got our own outhouse!",1);		
	} else if (status == 5) {	
	    qm.sendNextPrev("So, your name is #h0#, right? You have two options now. You can listen to some explanations about starting out, take a few small tests, get some free gifts, and become my new best friend in the entire world...",1);		
	} else if (status == 6) {	
	    qm.sendNextPrev("Or you can be teleported straight to town, but you'll miss out on my gifts... and I'll be super lonely and sad.",1);		
	} else if (status == 7) {
