...
function action(mode, type, selection) {
	...
	if (status == 1) {
		if (selection == 1) {
			...
			var em = cm.getEventManager("Autodbbaolv2");
			if (em == null) {
				...
				if (prop.equals("0") || prop == null) {
					...
					cm.worldMessage(5,"["+cm.getName()+"]双倍活动：已为全服开启只能在1线开启系统双倍爆率活动，持续8小时，要升级的赶紧了!");
					cm.dispose();
					return;
				} else {
			...
			if (em == null) {
				...
				if (prop.equals("0") || prop == null) {
					...
					//var 人气 = cm.getPlayer().getFame();
					//cm.getPlayer().setFame(人气+20);
					cm.worldMessage(1, "【双倍爆率活动】\r\n" + cm.getChar().getName() + "已为全服开启双倍爆率活动，持续1小时，要升级的赶紧了！");
					cm.dispose();
					return;
				} else {
					cm.sendOk("目前正在系统双倍爆率时间中...");
