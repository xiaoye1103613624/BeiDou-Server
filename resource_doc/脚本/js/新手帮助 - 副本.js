		...
		if (status == 0) {
			...
			switch (sele1) {
				case 1:
					...
					var 领取记录 = cm.getPlayer().getBossLog1("新手礼包");
					if (领取记录 != 0) {
						text += "\t\t\t\t\t\t您已经领取过礼包了！\r\n";
						cm.sendOk(text);
						cm.dispose();
				...
				case 3:
					...
					for (var i = 0; i < 等级礼包.length; i++) {
						var is_i = 等级礼包[i];
						var 领取记录 = cm.getPlayer().getBossLog1("新手等级礼包:" + is_i.要求等级);
				...
				case 4:
					var text = "#k";
					var 领取记录 = cm.getPlayer().getBossLog1("新手药水礼包");
					var 每日记录 = cm.getPlayer().getBossLog1("新手药水礼包_每日");
					text += " #k┏━#r冒险岛提示#k━━━━━━━━━━━━━━━━━━┓\r\n";
					text += "\t#k" + 广播 + " 欢迎来到[#r新手药水礼包#k]\r\n";
					//text += "\t#k" + 广播 + " [#r药水礼包#k]总共可以领取7次！\r\n";
					//text += "\t#k" + 广播 + " 领取记录：[#b" + 领取记录 + "#k/#r" + 7 + "#k]！\r\n";
					text += " #k┗━━━━━━━━━━━━━━━━━━━━━━━━┛#k\r\n";
					text += "" + 分割线() + "\r\n";
			...
			switch (sele1) {
				case 1:
					cm.getPlayer().setBossLog1("新手礼包",1);//永久LOG
					cm.gainDY(新手礼包.抵用);
					for (var i = 0; i < 新手礼包.物品.length; i++) {
						var is_i = 新手礼包.物品[i]
				...
				case 3:
					...
					text += "\r\n";
					var 领取记录 = cm.getPlayer().getBossLog1("新手等级礼包:" + 选择礼包.要求等级);
					...
					break;
				case 4:
					cm.getPlayer().setBossLog1("新手药水礼包",1);//永久LOG
					cm.getPlayer().setBossLog1("新手药水礼包_每日",1);//每日LOG
					for (var i = 0; i < 药水礼包.length; i++) {
						var is_i = 药水礼包[i];
						cm.gainItem(is_i.代码, is_i.数量, is_i.时限);
			...
			switch (sele1) {
				...
				case 3:
					var 选择礼包 = 等级礼包[sele2];
					cm.getPlayer().setBossLog1("新手等级礼包:" + 选择礼包.要求等级,1);
					cm.gainDY(选择礼包.抵用);
					for (var i = 0; i < 选择礼包.物品.length; i++) {
						var is_i = 选择礼包.物品[i];
						cm.gainItem(is_i.代码, is_i.数量, is_i.时限);
