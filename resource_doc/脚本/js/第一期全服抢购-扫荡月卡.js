...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "#d本期的宝物：#r#i" + 商品代码 + "# #z" + 商品代码 + "#\r\n打开可获得：#v2022512:#  #z2022512# #b30天使用权\r\n\r\n";
            text += ""+红色时钟+" #d剩余数量：#r#e" + 商品还有没 + "/#b100 #d#n份 "+警报灯+"\r\n\r\n";
            text += ""+红色时钟+" #d物品单价：#r#e" + 价格是多少 + "#d#n 赞助#d "+警报灯+"  当前余额: #b#e" + cm.getPlayer().getmoney() + " #n#k\r\n\r\n";
			text += ""+蓝色时钟+" #d购买须知：一键扫荡月卡，每月只能购买#r#e1#n#d次\r\n\r\n";
		...
		//	if(cm.getChar().isGM()){
            ...
            if (selection == 1) {
				if (cm.getPlayer().GetCombat() < 100000) {
					cm.sendOk("购买失败！你现在太弱了，战力提升到100万再来吧！");
					cm.dispose();
					return;
				}
                if (商品还有没 <= 0) { 
                ...
                //    cm.getPlayer().setOneTimeLog(冠名);
                    ...
                    cm.sendOkS("恭喜您，抢购成功 获得#i" + 商品代码 + "# #z" + 商品代码 + "# ", 2);
					cm.全服漂浮喇叭( "【全服抢购】恭喜土豪玩家 "+cm.getPlayer().getName()+" 在全服抢购活动中，抢到了 【一键扫荡月卡】 ！" , 5120015 );
					cm.喇叭(2, "恭喜土豪玩家[" + cm.getName() + "]:在全服抢购活动中，抢到了 一键扫荡月卡 ！");
					Packages.tools.FileoutputUtil.log("log\\玩家相关\\全服抢购.log", "[" + cm.getName() + "]花费 " +价格是多少+ " 赞助，抢购了 " + 冠名 + " 一键扫荡卡 ");  //  记录日志
                    cm.dispose();
                }
