...
var fubendt = 109020001; // 定义副本地图ID变量

// 活动副本事件名称（与配置表中的事件名对应）
var eventName = "活动事件";  
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (em == null) {
            cm.sendOk("当前" + eventName + "事件未配置，无法进入！");
            cm.dispose();
            return;
        }
        // ==========================================

        if (cm.getPlayerCount(109020001) <= 0) {
            // 根据选择的挑战类型处理
            switch (selection) {
                case 0: // 普通挑战
                    if (cm.getPlayer().getBossLog("活动副本") >= 100) {
                        cm.sendOk("你今天的普通挑战次数已用完！");
                        cm.dispose();
                        return;
                    }
                    if (!cm.判断团队物品(4031227, 5) || !cm.判断团队物品(2381048, 1)) {
                        cm.sendOk("队伍中有人#v4031227#或#v2381048#不足，无法进入");
                        cm.dispose();
                        return;
                    }
                    cm.收团队道具(4031227, -5);
                ...
                case 1: // 高级挑战
                    if (cm.getPlayer().getBossLog("活动副本") >= 3) {
                    ...
                    cm.收团队道具(4031227, -5);
                    cm.收团队道具(2381048, -1);                   
                    cm.给团队每日("活动副本"); // 增加2次挑战次数
                    break;

                case 2: // 特级挑战
                    if (cm.getPlayer().getBossLog("活动副本") >= 6) {
            ...
            cm.warpParty(109020001, 0);
            cm.killAllMob(109020001); // 清理副本怪物
            cm.getPlayer().startMapTimeLimitTask(1800, cm.getChannelServer().getMapFactory().getMap(910000000));
            cm.spawnMobOnMap(8880803,1,126, -205, 109020001, jdHP);
            cm.喇叭(1, "【" + eventName + "】困难BOSS：[" + cm.getPlayer().getName() + "]带领他的队伍前往挑战桃园BOSS了！");
        } else {
