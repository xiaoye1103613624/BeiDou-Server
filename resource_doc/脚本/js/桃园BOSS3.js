...
var fubendt = 401070000; // 定义副本地图ID变量
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        text += "        [限时30分钟]\r\n";
        text += "        #b#k[每天进入次数]：#r" + cm.getPlayer().getBossLog('暗黑意念') + "#k/"+次数+"#l [等级要求]：120\r\n";
        ...
        if (cm.getPlayerCount(401070000) <= 0) {
            // 根据选择的挑战类型处理
            switch (selection) {
                case 0: // 普通挑战
                    if (cm.getPlayer().getBossLog("暗黑意念") >= 1) {
                ...
                case 1: // 高级挑战
                    if (cm.getPlayer().getBossLog("暗黑意念") >= 3) {
                    ...
                    if (!cm.判断团队物品(3700070, 1)|| !cm.判断团队物品(2381048, 1)|| !cm.判断团队物品(2381048, 1)) {
                     ...
                     cm.收团队道具(4031227, -5);
                    cm.收团队道具(3994789, -1);                   
                    cm.给团队每日("暗黑意念"); // 增加2次挑战次数
                    break;

                case 2: // 特级挑战
                    if (cm.getPlayer().getBossLog("暗黑意念") >= 6) {
