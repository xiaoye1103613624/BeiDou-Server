...
function showRanking() {
    ...
    if (rankingData.length > 0) {
        ...
        if (filteredRankingData.length > 0) {
            var rankingText = "" + 群粉心 + "#e#r\t\t\t#i1802533#全服破功排行榜#i1802533##n\r\n";
            rankingText += "#k------------------------------------------------------\r\n";
            rankingText += "#r#e" + 粉心 + "排名" + 粉心 + "       " + 粉心 + "破功点数" + 粉心 + "      " + 粉心 + "玩家名称" + 粉心 + " #k#n\r\n";
            rankingText += "#k------------------------------------------------------\r\n";
            for (var i = 0; i < filteredRankingData.length; i++) {
                var playerName = pad(filteredRankingData[i].name, 7); // 格式化玩家名称，固定宽度为7
                var damage = pad(filteredRankingData[i].damage.toString(), 10); // 格式化破功点数，固定宽度为10
                var rankNumber = pad((i + 1).toString(), 2); // 格式化排名，固定宽度为2
                var rankColor = i < 3 ? "#r" : "#k"; // 前三名用红色字体，其他用默认字体
                rankingText += rankColor + "   " + rankNumber + "             " + damage + "         " + playerName + "  \r\n";
            }
...
function getRankingData() {
    ...
    var sql = "SELECT name, damage FROM " + CHARACTERS_TABLE + " WHERE gm = 0 ORDER BY damage DESC LIMIT 20;";  //这里的数字改为多少级显示多少条
