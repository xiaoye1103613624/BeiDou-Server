		...
		if (selection == 1) {
			...
			if (em == null) {
				...
				if (prop.equals("0") || prop == null) {
					...
					cm.worldMessage(5,"["+cm.getName()+"]双倍活动：已为全服开启只能在1线开启系统双倍经验活动，持续2小时，要升级的赶紧了!");
					cm.dispose();
					return;
				} else {
			...
			if (em == null) {
				...
				if (prop.equals("0") || prop == null) {
					...
					//var 人气 = cm.getPlayer().getFame();
					//cm.getPlayer().setFame(人气+20);
					cm.worldMessage(9, "【双倍活动】\r\n" + cm.getChar().getName() + "已为全服开启双倍经验活动，持续2小时，要升级的赶紧了！");
					cm.dispose();
					return;
				} else {
					cm.sendOk("目前正在系统双倍时间中...");
