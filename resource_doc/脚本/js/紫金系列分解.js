...
var item分解池 = [
    ...
    {id: 1092022, 积分: 200}  // T5盾牌
	
/*	{id: 1012752, 积分: 2},  // T5面饰
	{id: 1122163, 积分: 2},  // T5项链
	{id: 1032327, 积分: 2},  // T5耳环
	{id: 1022313, 积分: 2} */  // T5眼镜
        ...
        if (status == 0) {
            ...
            if (selection == 0) {
                ...
                //    cm.sendOk("分解成功！您获得了#r" + 总积分 + "#k积分\r\n当前积分剩余：#r" + 查询当前积分() + "#k");
                    cm.喇叭(3, "恭喜玩家：[" + cm.getPlayer().getName() + "]成功分解 T5系列装备 并获得 " + 总积分 + " 积分！");
...
function 更改积分(sum) {
    ...
    if (sum >= 1) {
        cm.getPlayer().dropMessage(5, "获得：" + sum + " 积分(分解T5系列装备)，当前积分剩余：" + 查询当前积分() + "");
    } else if (sum < 0) {
        cm.getPlayer().dropMessage(5, "消费：" + sum + " 积分(分解T5系列装备)，当前积分剩余：" + 查询当前积分() + "");
    }
}
