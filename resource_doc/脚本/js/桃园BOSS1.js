...
var fubendt = 811000500; // 定义副本地图ID变量
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        text += "        [限时30分钟]\r\n";
        text += "        #b#k[每天进入次数]：#r" + cm.getPlayer().getBossLog('鸿蒙巨人') + "#k/"+次数+"#l [等级要求]：120\r\n";
        ...
        if (cm.getPlayerCount(811000500) <= 0) {
            // 根据选择的挑战类型处理
            switch (selection) {
                case 0: // 普通挑战
                    if (cm.getPlayer().getBossLog("鸿蒙巨人") >= 1) {
                ...
                case 1: // 高级挑战
                    if (cm.getPlayer().getBossLog("鸿蒙巨人") >= 3) {
                        cm.sendOk("你今天的高级挑战次数已用完！");
                        cm.dispose();
                        return;
                    }
                    if (!cm.判断团队物品(3700070, 1)|| !cm.判断团队物品(3994789, 1)|| !cm.判断团队物品(3994789, 1)) {
                    ...
                    cm.收团队道具(4031227, -5);
                    cm.收团队道具(3994789, -1);                   
                    cm.给团队每日("鸿蒙巨人"); // 增加2次挑战次数
                    break;

                case 2: // 特级挑战
                    if (cm.getPlayer().getBossLog("鸿蒙巨人") >= 6) {
                        cm.sendOk("你今天的特级挑战次数已用完！");
                        cm.dispose();
                        return;
                    }
                    if (!cm.判断团队物品(3700069, 1)|| !cm.判断团队物品(3994789, 1)|| !cm.判断团队物品(3994789, 1)) {
                        cm.sendOk("队伍中有人道具不足，无法进入");
                        cm.dispose();
                        return;
                    }
            ...
            cm.warpParty(811000500, 0);
            cm.killAllMob(811000500); // 清理副本怪物
            cm.getPlayer().startMapTimeLimitTask(1800, cm.getChannelServer().getMapFactory().getMap(910000000));
            cm.spawnMobOnMap(9601221,1,126, -205, 811000500, jdHP);
