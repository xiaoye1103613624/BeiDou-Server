        ...
        if (status == 0) {
           ...
           // }else{
              //      cm.dispose();
                    ...
                    if(cm.getPlayer().getSkillLevel(第6个代码)<第6个代码加点){
					text += " #L6# [进阶 LVL"+第6个代码所需+" < - > "+第6个代码加点+"级 #s"+第6个代码+"#]#l\r\n\r\n"//3
}
                    if(cm.getPlayer().getSkillLevel(第7个代码)<第7个代码加点){
					text += " #L7# [进阶 LVL"+第7个代码所需+" < - > "+第7个代码加点+"级 #s"+第7个代码+"#]#l\r\n\r\n"//3
}
                    if(cm.getPlayer().getSkillLevel(第8个代码)<第8个代码加点){
					text += " #L8# [进阶 LVL"+第8个代码所需+" < - > "+第8个代码加点+"级 #s"+第8个代码+"#]#l\r\n\r\n"//3
           }else{
                //    cm.dispose();
}
	...
	//	}else if (cm.getPlayer().getOneTimeLog(""+第2个代码+"") >0) {
     ...
     //               status = -1;
 		 ...
 		 if (cm.haveItem(材料1, 材料1数量) == false) {
                    ...
                    cm.sendOk(""+任务大白ms3116519632+"\r\n#r材料不符合突破技能条件,[代表自身背包材料总数/任务所需]#k#n\r\n需要1,#i"+材料1+"# [#c"+材料1+"#/#r"+材料1数量+"#k]\r\n需要2,#i"+材料2+"# [#c"+材料2+"#/#r"+材料2数量+"#k]\r\n需要3,#i"+材料3+"# [#c"+材料3+"#/#r"+材料3数量+"#k]\r\n金币4,#i2140000#:["+cm.getMeso()+"/#r"+金币所需+"#k]\r\n\r\n元宝5, #d[ 当前元宝：#r"+cm.getmoneyb()+" / 所需：#r30 元宝#n#d ]\r\n\r\n");
                    status = -1;
		}else if (cm.getPlayer().getSkillLevel(第7个代码) <第7个代码所需) {
                    cm.sendOk("本技能没有到达"+第7个代码所需+"级无法进行突破");
                    status = -1;
	//	}else if (cm.getPlayer().getOneTimeLog(""+第2个代码+"") >0) {
     //               cm.sendOk("本技能没有到达 "+第2个代码所需+" 级无法进行突破");
     //               status = -1;
                    ...
                    cm.gainItem(材料3,-材料3数量);
					cm.setmoneyb(-30)				
                    cm.teachSkill(第7个代码,第7个代码加点,第7个代码加点);
                    cm.getPlayer().setOneTimeLog(""+第7个代码+"");
                    cm.sendOk("进阶成功，技能已为你突破！");
                    cm.dispose();
			}
			         }else if (selection == 8) {
 		 if (cm.haveItem(材料1, 材料1数量) == false) {
                    ...
                    cm.sendOk(""+任务大白ms3116519632+"\r\n#r材料不符合突破技能条件,[代表自身背包材料总数/任务所需]#k#n\r\n需要1,#i"+材料1+"# [#c"+材料1+"#/#r"+材料1数量+"#k]\r\n需要2,#i"+材料2+"# [#c"+材料2+"#/#r"+材料2数量+"#k]\r\n需要3,#i"+材料3+"# [#c"+材料3+"#/#r"+材料3数量+"#k]\r\n金币4,#i2140000#:["+cm.getMeso()+"/#r"+金币所需+"#k]\r\n\r\n元宝5, #d[ 当前元宝：#r"+cm.getmoneyb()+" / 所需：#r30 元宝#n#d ]\r\n\r\n");
                    status = -1;
		}else if (cm.getPlayer().getSkillLevel(第8个代码) <第8个代码所需) {
                    cm.sendOk("本技能没有到达"+第8个代码所需+"级无法进行突破");
                    status = -1;
	//	}else if (cm.getPlayer().getOneTimeLog(""+第2个代码+"") >0) {
     //               cm.sendOk("本技能没有到达 "+第2个代码所需+" 级无法进行突破");
     //               status = -1;
                    ...
                    cm.gainItem(材料3,-材料3数量);
					cm.setmoneyb(-30)				
                    cm.teachSkill(第8个代码,第8个代码加点,第8个代码加点);
                    cm.getPlayer().setOneTimeLog(""+第8个代码+"");
                    cm.sendOk("进阶成功，技能已为你突破！");
                    cm.dispose();
			}
