...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			var Eq = im.getInventory(1).getItem(1);
			if (Eq == null) {
				im.sendOk("#e#d使用该功能！\r\n必须把要强化的装备放置在装备栏第一个格！");
				im.dispose();
				return;
			} else if (banItemId(Eq.getItemId()).bool == false) {
				im.sendOk("#e#d以下装备不能参与强化:\r\n" + banItemId(Eq.getItemId()).text);
				im.dispose();
				return;
			} else if (im.isCash(Eq.getItemId()) == true) {
				im.getPlayer().dropMessage(1, "点卷时装不能强化的！");
				im.dispose();
				return;
			} else if (Eq.getUpgradeSlots() == 0) {
				im.getPlayer().dropMessage(1, "装备没有可升级次数了！");
				im.dispose();
				return;
			} else {
			...
			switch (sele) {
				case 1:
					if (物品数量(强化消耗.代码) < 强化消耗.数量) {
						im.sendOk("#e#v" + 强化消耗.代码 + ":##b#z" + 强化消耗.代码 + "##d 不足：#r" + 强化消耗.数量 + "#d张，无法强化！");
						im.dispose();
						return;
					}
					if (祝福卷轴机制 == true) {
						if (物品数量(祝福卷轴) < 1) {
							im.sendOk("#e#v" + 祝福卷轴 + ":##b#z" + 祝福卷轴 + "##d 不足：#r" + 1 + "#d张，无法使用该功能！");
							status = -1;
							return;
						}
					}
					...
					im.gainItem(强化消耗.代码, -强化消耗.数量);
					var random = Math.floor(Math.random() * 100) + 1;
					var item = im.getInventory(1).getItem(1);
					if (random <= 成功概率) {
						...
						im.getPlayer().forceReAddItem_Flag(item.copy(), Packages.client.inventory.MapleInventoryType.EQUIP);
						im.sendOk("#e#d恭喜您！强化成功了。");
						status = -1;
						return;
					} else {
						if (祝福卷轴机制 == false) {
...
function 物品数量(itemid) {
	return im.getPlayer().getItemQuantity(itemid, false);
}
