        ...
        if (status == 0) {
            ...
            if (selection == 0) {
               ...
               //     cm.sendOk("分解成功！您获得了#r" + 总积分 + "#k积分\r\n当前积分剩余：#r" + 查询当前积分() + "#k");
                    cm.喇叭(3, "恭喜玩家：[" + cm.getPlayer().getName() + "]成功分解 B级宝石戒指 并获得 " + 总积分 + " 积分！");
...
function 更改积分(sum) {
    ...
    if (sum >= 1) {
        cm.getPlayer().dropMessage(5, "获得：" + sum + " 积分(分解B级宝石戒指)，当前积分剩余：" + 查询当前积分() + "");
    } else if (sum < 0) {
        cm.getPlayer().dropMessage(5, "消费：" + sum + " 积分(分解B级宝石戒指)，当前积分剩余：" + 查询当前积分() + "");
    }
}
