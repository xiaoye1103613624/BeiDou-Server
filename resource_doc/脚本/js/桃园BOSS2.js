...
var fubendt = 401060000; // 定义副本地图ID变量
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        text += "        [限时30分钟]\r\n";
        text += "        #b#k[每天进入次数]：#r" + cm.getPlayer().getBossLog('小龙') + "#k/"+次数+"#l [等级要求]：120\r\n";
        ...
        if (cm.getPlayerCount(401060000) <= 0) {
            // 根据选择的挑战类型处理
            switch (selection) {
                case 0: // 普通挑战
                    if (cm.getPlayer().getBossLog("小龙") >= 1) {
                ...
                case 1: // 高级挑战
                    if (cm.getPlayer().getBossLog("小龙") >= 3) {
                    ...
                    cm.收团队道具(4031227, -5);
                    cm.收团队道具(2381050, -1);                    
                    cm.给团队每日("小龙"); // 增加2次挑战次数
                    break;

                case 2: // 特级挑战
                    if (cm.getPlayer().getBossLog("小龙") >= 6) {
            ...
            cm.spawnMobOnMap(9410193,1,1307, -1347, 401060000, jdHP);
