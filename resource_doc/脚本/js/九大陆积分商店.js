...
*/
	{
		...
		介绍: [
			{ 编辑: "分解以下装备获得对应积分" },
			{ 编辑: "#v1302900:##r#z1302900#  每件[3000积分] " },
			{ 编辑: "#v1302908:##r#z1302908#  每件[3000积分] " },
			{ 编辑: "#v1302904:##r#z1302904#  每件[3000积分] " },
			{ 编辑: "#v1302905:##r#z1302905#  每件[3000积分] " },
			{ 编辑: "#v1302107:##r#z1302107#  每件[3000积分] " },
			{ 编辑: "#v1402142:##r#z1402142#  每件[3000积分] " },
			{ 编辑: "#v1402224:##r#z1402224#  每件[3000积分] " },
			{ 编辑: "#v1402184:##r#z1402184#  每件[3000积分] " },
			{ 编辑: "#v1352975:##r#z1352975#  每件[5000积分] " },
...
var 扩充 = { 类型: "点券", 价格: 388888, 扩充Log: "黑暗领域", 上限: 1 }; //世界树扩充 改了影响到BOSS门口NPC，要同步
...
var 分解 = [
	...
	{ 代码: 1352970, 积分: 2000 }, //2000积分
	
	{ 代码: 1302908, 积分: 3000 }, //3000积分
	{ 代码: 1302905, 积分: 3000 }, //3000积分
	{ 代码: 1302904, 积分: 3000 }, //3000积分
	{ 代码: 1302900, 积分: 3000 }, //3000积分
	{ 代码: 1402184, 积分: 3000 }, //3000积分
	{ 代码: 1402142, 积分: 3000 }, //3000积分
	{ 代码: 1402224, 积分: 3000 }, //3000积分
	{ 代码: 1302107, 积分: 3000 }, //3000积分
	{ 代码: 1352975, 积分: 5000 }, //3000积分
...
var 商城 = [
	{ 代码: 2350014, 价格: 188888, 数量: 1, 类型: "积分" },
	{ 代码: 2350014, 价格: 1888888, 数量: 10, 类型: "积分" },
	{ 代码: 2350014, 价格: 18888888, 数量: 100, 类型: "积分" },

	{ 代码: 3603000, 价格: 300000, 数量: 1, 类型: "积分" },
	{ 代码: 3603000, 价格: 3000000, 数量: 10, 类型: "积分" },
	
	{ 代码: 3602000, 价格: 900000, 数量: 1, 类型: "积分" },
	{ 代码: 3602000, 价格: 9000000, 数量: 10, 类型: "积分" },
	
	{ 代码: 3603006, 价格: 1500000, 数量: 1, 类型: "积分" },
	{ 代码: 3603006, 价格: 15000000, 数量: 10, 类型: "积分" },
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		//	text += "\t#d" + 广播 + " #k这里可以扩充1-8大陆所有BOSS挑战次数\r\n";
...
function 扩充次数B() {
    switch (sele1) {
        case 是:
			...
			var Log1 = cm.getPlayer().getBossLog("圣地庭院"); // 获取当前的扩充次数
			var Log2 = cm.getPlayer().getBossLog("神殿"); // 获取当前的扩充次数
			var Log3 = cm.getPlayer().getBossLog("云林天宫"); // 获取当前的扩充次数
			var Log4 = cm.getPlayer().getBossLog("魔法庭院"); // 获取当前的扩充次数
			var Log5 = cm.getPlayer().getBossLog("枫叶城堡"); // 获取当前的扩充次数
			var Log6 = cm.getPlayer().getBossLog("世界之树"); // 获取当前的扩充次数
			var Log7 = cm.getPlayer().getBossLog("维利塔斯"); // 获取当前的扩充次数
            if (Log >= 扩充.上限) {
                cm.getPlayer().dropMessage(5, "您已达扩充上限，无法继续扩充！");
            } else {
                扣除代价(扩充.类型, -扩充.价格);
                cm.getPlayer().setBossLog(扩充.扩充Log, Log + 1); // 增加扩充次数
				cm.getPlayer().setBossLog("圣地庭院", Log1 + 1); // 增加扩充次数
				cm.getPlayer().setBossLog("神殿", Log2 + 1); // 增加扩充次数
				cm.getPlayer().setBossLog("云林天宫", Log3 + 1); // 增加扩充次数
				cm.getPlayer().setBossLog("魔法庭院", Log4 + 1); // 增加扩充次数
				cm.getPlayer().setBossLog("枫叶城堡", Log5 + 1); // 增加扩充次数
				cm.getPlayer().setBossLog("世界之树", Log6 + 1); // 增加扩充次数
				cm.getPlayer().setBossLog("维利塔斯", Log7 + 1); // 增加扩充次数
                cm.getPlayer().dropMessage(5, "1-8大陆所有BOSS可挑战次数已增加1次！！！");
            }
            break;
        case 否:
            break;
