...
var 阿卡伊勒 = "#fUI/UIWindow.img/MobGage/Mob/8860000#";
function start() {
    status = -1;
    action(1, 0, 0);
}
    ...
    if (status == 0) {
      ...
      //      text += "当前队伍状况:#g 符合要求   #k\r\n\r\n"
        ...
        if (cm.getParty() == null) {
            ...
            cm.dispose();
            return;
		} else if (!cm.getParty每日记录("阿卡伊勒次数", cishuxianzhi)) { //判断组队是否2次
            ...
            if (inMap < minPartySize || inMap > maxPartySize) {
                cm.sendOk("你的队伍人数不足" + minPartySize + "人.请把你的队伍人员召集到阿卡伊勒门口再进入副本.");
                cm.dispose();
                return;
         ...
         //   } else if (cm.getPlayer().getClient().getChannel()== 3 && cm.getPlayer().getClient().getChannel() == 4){ 
             ...
             //    cm.dispose();
              //   return;
            } else if (cm.getPlayer().getBossLog("阿卡伊勒次数") >= cishuxianzhi) {
                ...
                var em = cm.getEventManager("AKYL");
                if (em == null) {
                    cm.sendOk("这台电脑是当前不可用.");
                } else {
                    if (cm.getPlayerCount(932100002) <= 0) {//判定地图人数
                        em.startInstance(cm.getParty(), cm.getPlayer().getMap());
						cm.giveParty每日记录("阿卡伊勒次数");
	            	    cm.召唤怪物(8860000, 99900000000, 10000000, 1, 932100002, 14,-181);   
                     //   AAA();//添加重返检测
                    } else {
