...
function showRanking() {
    ...
    if (rankingData.length > 0) {
        var rankingText = "" + 群粉心 + "#e#r\t\t\t#i1802533#全服元宝排行榜#i1802533##n\r\n";
        rankingText += "#k------------------------------------------------------\r\n";
        rankingText += "#r#e" + 粉心 + "排名" + 粉心 + "  " + 粉心 + "账号ID" + 粉心 + "   " + 粉心 + "元宝数量" + 粉心 + "  " + 粉心 + "玩家名称" + 粉心 + "#k#n\r\n";
        rankingText += "#k------------------------------------------------------\r\n";
        for (var i = 0; i < rankingData.length; i++) {
            var playerName = pad(rankingData[i].name, 7); // 格式化玩家名称，固定宽度为7
            var money = pad(rankingData[i].money.toString(), 6); // 格式化元宝数量，固定宽度为10
...
function getRankingData() {
	...
	var sql = "SELECT a.id AS account, c.name, a.moneyb AS money FROM accounts a LEFT JOIN characters c ON a.id = c.accountid WHERE c.gm = 0 GROUP BY a.id ORDER BY money DESC LIMIT 20";  // 显示前20名
