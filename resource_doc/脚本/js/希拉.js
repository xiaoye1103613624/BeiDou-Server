...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        text += "#k\t\t\t#r 做好挑战 " + fbmc + "" + 希拉 + " 了么 #k\r\n\r\n进入要求如下：\r\n①人数限制:#r " + minPartySize + " #b- #r" + maxPartySize + "#k队员\t②等级限制：#r " + minLevel + " #b- #r" + maxLevel + "级 #k\r\n"
        text += "#k每天只能挑战:#b" + cishuxianzhi + "#k次 你今天已进入:#b" + cm.getPlayer().get每日记录("希拉次数") + "#k次#k\r\n"
		text += "挑战BOSS需要3000w金币\r\n"
        text += "#k战力高于:#b40万#k才可进入[当前战力:#b"+cm.getPlayer().GetCombat()/10000+"万#k]#k\r\n"
        // text += "#k需要#v4001127#1个#k\r\n"
         if (!cm.getParty每日记录("希拉次数", cishuxianzhi) ) { //判断组队是否2次
      ...
      //      text += "当前队伍状况:#g 符合要求   #k\r\n\r\n"
        ...
        if (cm.getParty() == null) {
            ...
            if (inMap < minPartySize || inMap > maxPartySize) {
                cm.sendOk("你的队伍人数不足" + minPartySize + "人.请把你的队伍人员召集到希拉门口再进入副本.");
                cm.dispose();
                return;
             ...
             //    cm.dispose();
              //   return;
                ...
                if (em == null) {
                    ...
                    if (cm.getPlayerCount(481000000) <= 0) {//判定地图人数
                        em.startInstance(cm.getParty(), cm.getPlayer().getMap());
						cm.giveParty每日记录("希拉次数");
	            	    cm.召唤怪物(8870000, 25500000000, 10000000, 1, 481000000, -19,184);   
                     //   AAA();//添加重返检测
                    } else {
