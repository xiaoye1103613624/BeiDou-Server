...
function action(mode, type, selection) {
	...
	if (status == 0) {
		var txt = "";
		txt += "   \t\t  " + 心2 + "   " + 心2 + "  #d#e < 自助双爆 > #k#n  " + 心2 + "   " + 心2 + "\r\n\r\n";
		txt += "\tHi~#b#h ##k，这里是#b自助双倍爆率系统#k，如果你有足够的点券或抵用券的话，可以给全服开启双倍经验哦！另外，给全服开双的大佬将获得以下福利：\r\n"
		//txt += ""+奖励+"\r\n"
		//txt += "\t"+人气王+" 20点\t\t全属性5#v1122017##z1122017##r                                              （12小时权,戴上就增加30%额外经验值）\r\n\r\n";
		txt += "#L1#给全服开双爆 12 小时\r\n\r\n";
		//txt += "#L2#给全服开双爆 12 小时 （需要10000抵用券）";
		cm.sendOk(txt);
	} 
	if (status == 1) {
		if (selection == 1) {
			if (cm.getPlayer().getClient().getChannel() != 1) {
				cm.sendOk("只能在1线开启系统双倍爆率。");
				cm.dispose();
				return
			}
			if (cm.getPlayer().getCSPoints(1) > 1){
			var em = cm.getEventManager("Autodbbaolv3");
			if (em == null) {
				...
				if (prop.equals("0") || prop == null) {
					...
					cm.worldMessage(5,"["+cm.getName()+"]双倍活动：已为全服开启只能在1线开启系统双倍爆率活动，持续12小时，要升级的赶紧了!");
					cm.dispose();
					return;
				} else {
					cm.sendOk("目前正在系统双倍爆率时间中...");
				}
			}
			} else {
				cm.sendOk("你的点券不足.");
			...
			if (em == null) {
				...
				if (prop.equals("0") || prop == null) {
					...
					//var 人气 = cm.getPlayer().getFame();
					//cm.getPlayer().setFame(人气+20);
					cm.worldMessage(6, "【双倍爆率活动】\r\n" + cm.getChar().getName() + "已为全服开启双倍爆率活动，持续12小时，要升级的赶紧了！");
					cm.dispose();
					return;
				} else {
					cm.sendOk("目前正在系统双倍爆率时间中...");
