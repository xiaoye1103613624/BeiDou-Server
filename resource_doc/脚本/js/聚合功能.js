...
//var 群粉心 = "#fMap/Back/zerek/拍卖/标题1#";
...
var 小黄星 = "#fEffect/CharacterEff.img/1112925/0/0#";
...
function start() {
    status = -1;
    // 1. 退而求其次：白名单方式取消常用怪物卡 BUFF
    var monsterCardIds = [
    2382001, 2382005, 2382008, 2382010, 2382016,
    2382028, 2382040, 2382050, 2382052, 2383003,
    2383004, 2383006, 2383010, 2384039, 2384040, // 加爆率的小怪卡
    // 2388000 ~ 2388055 BOSS卡
/*    2388000, 2388001, 2388002, 2388003, 2388004, 2388005,
    2388006, 2388007, 2388008, 2388009, 2388010, 2388011,
    2388012, 2388013, 2388014, 2388015, 2388016, 2388017,
    2388018, 2388019, 2388020, 2388021, 2388022, 2388023,
    2388024, 2388025, 2388026, 2388027, 2388028, 2388029,
    2388030, 2388031, 2388032, 2388033, 2388034, 2388035,
    2388036, 2388037, 2388038, 2388039, 2388040, 2388041,
    2388042, 2388043, 2388044, 2388045, 2388046, 2388047,
    2388048, 2388049, 2388050, 2388051, 2388052, 2388053,*/
    2388054, 2388055
	];
    for (var i = 0; i < monsterCardIds.length; i++) {
        cm.getPlayer().dispelBuff(monsterCardIds[i]);
    }
    // 检测是否在频道8
    ...
    if (currentChannel == 8) {
        ...
        if (!cm.haveItem(5220002)) {
			cm.getPlayer().dropMessage(5, "对不起，你缺少特殊物品，无法进入8频道，已为你切换至1频道");
			cm.getPlayer().dropMessage(5, "对不起，你缺少特殊物品，无法进入8频道，已为你切换至1频道");
			cm.getPlayer().dropMessage(5, "对不起，你缺少特殊物品，无法进入8频道，已为你切换至1频道");
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "#w\t\t\t"+Logo+"\r\n"+群粉心+"";
    text += "  #d" + 红方 + "玩家名称: #r" + padString(cm.getPlayer().getName(), 12) + "\r\n";
	text += "  #d" + 蓝方 + "在线时间: #r" + padString(cm.getPlayer().getTodayOnlineTime() + "#d 分钟", 12) + "  " + 蓝方 + "#d赞助余额: #r" + padString(cm.getPlayer().getmoney() + "#d 点", 4) + "\r\n";
    text += "  #d" + 绿方 + "当前元宝: #r" + padString(cm.getmoneyb() + "#d 点", 12) + "   " + 橙方 + "#d当前累计: #r" + padString(cm.getPlayer().getlpjf() + "#d 点", 4) + "\r\n";
    text += "  #d" + 黄方 + "拥有点卷: #r" + padString(cm.getPlayer().getCSPoints(1) + "#d 点", 13) + "  " + 紫方 + "#d当前破功: #r" + padString(cm.getPlayer().getDamage() + "#d 点", 12) + "\r\n"+群粉心+"";
  text +="           #L24#"+蓝色小兔子+"  赞  助  中  心  "+蓝色小兔子+"#l\r\n\r\n";
  text +="           #b#e#L26#"+蓝心+"游戏群号983146254"+蓝心+"#l  \r\n\r\n";
   text +="#b#e#L27#"+黑桃1+"首充礼包"+黑桃1+"#l #d#L28#"+黑桃1+"世界等级"+黑桃1+"#l #b#e#L29#"+黑桃1+"高级抽奖"+黑桃1+"#l"
   text +="#k    \r\n\r\n"+分割线3+"\r\n\r\n";
			...
			text +="#n  #L1#"+小黄星+"快捷传送"+小黄星+"#l  #L4#"+小黄星+"新手礼包"+小黄星+"#l\r\n\r\n";
			
			text +="#d#L3#"+小黄星+"快捷商店"+小黄星+"#l  #L8#"+小黄星+"每日双倍#d"+小黄星+"#l  #L7#"+小黄星+"任务中心"+小黄星+"#l\r\n\r\n";
			
			text +="#d#L20#"+小黄星+"查询暴率"+小黄星+"#l  #L14#"+小黄星+"装备分解"+小黄星+"#l  #L11#"+小黄星+"远征邮件"+小黄星+"#l\r\n\r\n";

			text +="#d#L9#"+小黄星+"#r随身仓库#d"+小黄星+"#l  #L21#"+小黄星+"删除物品"+小黄星+"#l  #d#L15#"+小黄星+"#r飞升渡劫"+小黄星+"#l\r\n\r\n";
			text +="                   #d#L116#"+小黄星+"回收赞助"+小黄星+"#l  \r\n\r\n";			
			text +="#k    \r\n"+群粉心+"";
			
			text +="#d#L887#"+游戏攻略+"#l#L888#"+CDKEY兑换+"#l\r\n" 

            // GM检测：只有GM级别大于99的用户才能看到以下内容
			if (cm.getPlayer().getGMLevel() > 99) {
				text += "\r\n#r--------------------GM选项--------------------#k\r\n";
                text += "#L117#"+小黄星+"充值面板"+小黄星+"#l #L19#"+小黄星+"巡查面板"+小黄星+"#l\r\n\r\n";
            }
        ...
        } else if (selection == 2) {//匠人街
		    cm.warp(910001000,0);
			cm.dispose();
		
     //   } else if (selection == 13) {//挑战大厅
			//cm.warp(960000000,0);
		//    cm.dispose();
		...
		} else if (selection == 11) { //保存邮件
            cm.openNpc(9000444,"保存邮件");	
		
        } else if (selection == 12) {//赞助系统
            cm.openNpc(9900004, "理财中心");
		
        } else if (selection == 13) {//卡片收集
		...
		} else if (selection == 16) {//戒指收集
            cm.openNpc(9000443,0);
		...
		} else if (selection == 19) {//美容美发
            cm.openNpc(9900004, "巡查面板");			
            ...
            cm.openNpc(9900004, "全服世界BOSS");	
		
		} else if (selection == 23) {//VIP练功房
            cm.openNpc(9900004, "VIP练功房");	
		
		} else if (selection == 24) {//管理面板
            cm.openNpc(9201429, 0);	
		} else if (selection == 116) {//VIP练功房
            cm.openNpc(9000445, "经验卡回收");	
		} else if (selection == 117) {//VIP练功房
            cm.openNpc(9900004, "kaixin管理面板");	
		} else if (selection == 27) {//首充礼包
            cm.openNpc(9900004, "首充礼包");	
		} else if (selection == 28) {//世界等级
            cm.openNpc(9900004, "世界等级");		
		} else if (selection == 29) {//高级抽奖机
            cm.warp(910023300,0);
			cm.dispose();			
			...
			for (var i = 1; i <= 5; i++) {
				if (cm.getInventory(i).isFull(1)) {
					cm.getPlayer().dropMessage(1, "背包所有栏位至少2个空位\r\n");
					cm.dispose();
					return;
				}
			}
		...
		} else if (selection == 100) { // GM专用：传送回自由市场
            if (cm.getPlayer().getGMLevel() > 99) {
                cm.warp(910000000, 0);
                cm.dispose();
            } else {
                cm.sendOk("权限不足，此功能仅限GM使用！");
                cm.dispose();
            }
