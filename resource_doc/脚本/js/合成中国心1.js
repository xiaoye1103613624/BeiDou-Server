...
var weapon = new Array(
	...
	Array(4000464,1102207,1,1,1,1,1,1)
...
function action(mode, type, selection) {
    ...
    if (cm.getBossRank("满足条件",2) > 0) {
       ...
       if(cm.haveItem(req[ii][0], req[ii][1])){
		...
		//msg += "\t\t";
       if(cm.getMeso() < rem){
		msg += "\r\n需要金币：[#r"+金币余额+"#k/"+rem+"#d 未满足"+W+"n#k";
    }else{
		msg += "\r\n 需要金币：[#r"+金币余额+"#k/"+rem+"#r 已满足"+X+"#n#k]";
    }
        msg += "";
        msg += "\r\n#g----------------------------------------------\r\n";
       ...
       cm.sendSimple("#d" + msg + "");
        ...
        cm.sendYesNo("#b是否要合成#r #i" + weapon[sels][0] + ":##z" + weapon[sels][0] + "# \r\n");
    } else if (status == 2) {
		...
		if(s1 <= gailv){
			...
			cm.sendNext("#b成功合成了 #i" + weapon[sels] + "##z" + weapon[sels]+ "#");
			cm.dispose();
		} else {
			for (var i = 0; i < req.length; i++) {
				cm.gainItem(req[i][0], -req[i][1]);
