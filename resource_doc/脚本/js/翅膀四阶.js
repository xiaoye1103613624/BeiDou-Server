...
var itemObjS = {
	"1102386": [[4003005, 2000],[4003004, 2000],[4001006, 60],[4032056, 30], [4170007, 16],['金币', 200000000], ['点券', 80000]],
	"1102385": [[4003005, 2000],[4003004, 2000],[4001006, 60],[4032056, 30], [4170007, 16],['金币', 200000000], ['点券', 80000]],
	"1102604": [[4003005, 2000],[4003004, 2000],[4001006, 60],[4032056, 30], [4170007, 16],['金币', 200000000], ['点券', 80000]],
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var text = "你想拥有翅膀吗???听说在本岛帅气的人都拥有属于自己的翅膀哦~!\r\n";
            text += "当前翅膀等阶:[#r翅膀三阶#k]\r\n";
            text += "请选择你需要制作点初级翅膀[#r全属性+40#k]\r\n";
		for(var oKey in itemObjS) {
			text += "#L"+ oKey +"#兑换#v"+ oKey +"##b说明:#k[#r全属性+40#k]#k\r\n";
		}
        cm.sendSimple(text);
    } else if (status == 1) {
		...
		cm.给属性装备(selected,0,0,40,40,40,40,0,0,40,40,0,0,0,0,0,0,0);
		cm.sendOk("进阶 #v"+ selected +"##b#z"+ selected +"##k成功。");
		cm.getPlayer().指定喇叭("高质地喇叭", "系统公告", "恭喜[" + cm.getPlayer().getName() + "]成功完成翅膀4阶!");
		cm.dispose();
	}
}